...
static char vcid[] = "$Id: func_surf_energy_bal.c,v 4.1 2000/05/16 21:07:16 vicadmin Exp $";
...
/* Start ngmod change */
/* char arguments are padded by va arg to ints */
  SNOWING             = va_arg(ap, int);
  FS_ACTIVE           = va_arg(ap, int);
/* end ngmod change */
  TMean = Ts;
...
#else
    /* wade's fix for the storage */
    *deltaH = Cs1 * (T1_old - *T1) * D1 / delta_t;
#endif
    /** JS -- end of changes **/
#if 1
    /** JS -- start of changes **/
    /* there is actually no storage for the soil surface interface so delthaH = 0.0 */
    *deltaH = 0.0;
    /** JS -- end of changes **/
#endif
    /** JS -- start of changes **/
    /* add a check that we are using frozen soils */

...
C**** CRITICAL VALUE 2: AVERAGE MOISTURE IN ROOT ZONE WHEN WMIN
C**** IS EXACTLY AT WILTING POINT.
        RZEQW=(WILT-1.-(2./AX))*EXP(-AX*(1.-WILT)) + WILT + (2./AX)

C**** SITUATION 1: CATDEF LE CDCR1
...
C**** SITUATION 2: CATDEF GT CDCR1
c        IF(CATDEF(N) .GT. CDCR1(N)) THEN
c          AR2(N)=1.-AR1(N)
c          FACTOR=(CDCR2(N)-CATDEF(N)-RZEXC(N))/(CDCR2(N)-CDCR1(N))
c          FACTOR=AMIN1(1., AMAX1(0., FACTOR))
c          X0=(RZEQW-WILT)/(RZEQX-WILT)
c          IF(FACTOR.LT.X0) THEN
c            AR1(N)=AR1(N)*FACTOR
c            AR2(N)=AR2(N)*FACTOR
c            AR4(N)=1.-FACTOR
c            ENDIF
c          ENDIF
...
c           IF(RZEQY .LT. WILT-.001) THEN
c             WRITE(*,*) 'RZEXC(N) WAY TOO LOW!: N=',N,'RZEXC=',RZEXC(N)
c           ELSE
c             WRITE(*,*) 'RZEXC(N) TOO LOW: N=',N
c           END IF
            RZEQY=WILT
...
C**** EXTRAPOLATION OF THE SURFACE WETNESSES

c 1st step: surface wetness in the unstressed fraction without considering
c           the surface excess; we just assume an equilibrium profile from 
c           the middle of the root zone to the surface.

        IF (SWSRF2(N) .NE. 0.) THEN 
          SWSRF2(N)=((SWSRF2(N)**(-BEE(N))) - (.5/PSIS(N)))
     &           **(-1./BEE(N))
          ENDIF
c if swsrf2 is 0. in the RZ, it is also 0. at the surface (and it corresponds 
c to ar2=0.)
     ...
     &           **(-1./BEE(N))
          ENDIF
c if swsrf4 is 0. in the RZ, it is also 0. at the surface(and it corresponds 
c to ar4=0.)

c srfmx is the maximum amount of water that can be added to the surface layer
c The choice of defining SWSRF4 like SWSRF2 needs to be better examined.
        srfmx(n)=ar2(n)*(1.-swsrf2(n))*(20.*poros(n))
        srfmx(n)=srfmx(n)+ar4(n)*(1.-swsrf4(n))*(20.*poros(n))
c**** For calculation of srfmn, assume surface moisture associated with
c**** AR1 is constantly replenished by water table.

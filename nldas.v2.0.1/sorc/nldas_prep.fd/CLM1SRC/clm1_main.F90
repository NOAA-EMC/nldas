...
!
!   Soil information
!     csol,watsat,sucsat,bsw,tkmg,tksatu,tkdry,hksat,wtfact
!
...
!=========================================================================
! $Id: clm1_main.F90,v 1.1.1.1 2003/02/06 16:10:44 jgottsch Exp $
!=========================================================================

  use precision
  use clm1type
  use clm1_varcon, only : istsoil, istwet, istice, denice, denh2o
  implicit none
...
! -----------------------------------------------------------------
  ...
  call clm1_dynvegpar (clm1)

! -----------------------------------------------------------------
! Albedos 
! -----------------------------------------------------------------

  call clm1_coszen (clm1, day, coszen)

  call clm1_surfalb (clm1, coszen)

! -----------------------------------------------------------------
! Surface Radiation
! -----------------------------------------------------------------

  call clm1_surfrad (clm1)

! -----------------------------------------------------------------
! Initial set of previous time step variables and water balance
! -----------------------------------------------------------------

  clm1%h2osno_old = clm1%h2osno  ! snow mass at previous time step
  clm1%h2ocan_old = clm1%h2ocan  ! depth of water on foliage at previous time step
  if (.not.clm1%lakpoi) then
     do j = 1,nlevsoi
        clm1%h2osoi_liq_old(j)=clm1%h2osoi_liq(j)+clm1%h2osoi_ice(j)
     enddo
     do j = clm1%snl+1, 0       ! ice fraction of snow at previous time step
        clm1%frac_iceold(j) = clm1%h2osoi_ice(j)/(clm1%h2osoi_liq(j)+clm1%h2osoi_ice(j)) 
     enddo
  endif
...
! determine beginning water balance (water balance at previous time step)
  ...
  clm1%begwb = clm1%h2ocan + clm1%h2osno
  do j = 1, nlevsoi
     clm1%begwb = clm1%begwb + clm1%h2osoi_ice(j) + clm1%h2osoi_liq(j)
  enddo
...
!     print*, clm1%lai_t1_f,clm1%lai_t2_f,clm1%sai_t1_f,clm1%sai_t2_f

     call clm1_hydro_canopy (clm1)

! Determine thermal processes and surface fluxes

     call clm1_thermal (clm1)

! Determine the change of snow mass and the snow water onto soil

     call clm1_hydro_snow (clm1)

! Determine Soil hydrology 

     if (clm1%itypwat == istsoil) call clm1_hydro_soil (clm1)
...
! Natural compaction and metamorphosis. The compaction rate is recalculated every timestep.
        ...
        call clm1_compact (clm1)

! Combine thin snow elements

        call clm1_combin (clm1)

! Divide thick snow elements

        call clm1_subdiv (clm1)

! Set zero to the empty node

        if (clm1%snl > -nlevsno) then
           clm1%snowage = 0.
           do j = -nlevsno+1, clm1%snl
              clm1%h2osoi_ice(j) = 0.
              clm1%h2osoi_liq(j) = 0.
              clm1%t_soisno(j) = 0.
              clm1%dz(j) = 0.
...
! Update ground temperature
     ...
     clm1%t_grnd = clm1%t_soisno(clm1%snl+1)

! Irrigate crops if appropriate
     
     if (clm1%itypwat == istsoil) call clm1_hydro_irrig (clm1)

! Determine volumetric soil water

     do j = 1,nlevsoi
        clm1%h2osoi_vol(j) = clm1%h2osoi_liq(j)/(clm1%dz(j)*denh2o) &
                          + clm1%h2osoi_ice(j)/(clm1%dz(j)*denice)
     end do

! Determine ending water balance for non-lake points

     clm1%endwb=clm1%h2ocan+clm1%h2osno
     do j = 1, nlevsoi
        clm1%endwb = clm1%endwb + clm1%h2osoi_ice(j) + clm1%h2osoi_liq(j)
     enddo

! Determine wetland and land ice hydrology (must be placed here since need snow 
! updated from clm1_combin) and ending water balance

     if (clm1%itypwat==istwet .or. clm1%itypwat==istice) call clm1_hydro_wetice (clm1)

! -----------------------------------------------------------------
! Energy AND Water balance for lake points
! -----------------------------------------------------------------

  else if (clm1%lakpoi) then

     call clm1_lake (clm1)

     do j = 1,nlevsoi
        clm1%h2osoi_vol(j) = 1.0
     end do
...
! -----------------------------------------------------------------
  ...
  call clm1_snowage (clm1)

! -----------------------------------------------------------------
! Check the energy and water balance
! -----------------------------------------------------------------

  call clm1_balchk (clm1)

  return
end subroutine clm1_main

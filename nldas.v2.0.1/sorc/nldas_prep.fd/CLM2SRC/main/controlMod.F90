...
!=======================================================================
  ...
  subroutine control_init (ldas, cam_caseid , cam_ctitle, cam_irad , cam_nsrest, &
...
! 
!-----------------------------------------------------------------------
! $Id: controlMod.F90,v 1.1.1.1 2003/02/06 16:10:50 jgottsch Exp $
!-----------------------------------------------------------------------
...
! ------------------------ local variables -----------------------------
    ...
    character(len=256) :: cecode    !used in restart filename in restFileMod.F90
...
! === define run =======================
!
!    o caseid     = 16 character case name
...
!=== Equate CLM2 date variables to LDAS date
    dtime=ldas%ts
    start_ymd = ldas%syr*10000  + ldas%smo*100  + ldas%sda
    start_tod = (ldas%shr*3600) + (ldas%smn*60) + ldas%sss
    stop_ymd  = ldas%eyr*10000  + ldas%emo*100  + ldas%eda
    stop_tod  = (ldas%ehr*3600) + (ldas%emn*60) + ldas%ess
...
! control variables
    ...
    write(cecode,'(i3.3)') ldas%expcode
    caseid  = cecode
    ctitle  = ' '
!===Set to assure CLM2 internal restart routines not called, may need to be used later
    if (ldas%startcode == 3) nsrest = 0
    if (ldas%startcode == 1) nsrest = 1    
...
#if (defined OFFLINE) && (defined RTM)
    rtm_nsteps =  1
#elif (defined COUP_CSM) && (defined RTM)
    rtm_nsteps =  9
#endif
...
!===Do not want to read in namelist parameters from standard input, set in ldas.crd
!!   open(77,file="lnd.stdin",status='old')
!!    if (masterproc) then
!!       read(77, clmexp, iostat=ierr)
!!       if (ierr /= 0) then
!!          if (masterproc) then
...
!===Do not want to make surface dataset in LDAS, have our own
!!    if (fsurdat == ' ') then
!!       mkfsurdat = .true.
!!    else
       mkfsurdat = .false.

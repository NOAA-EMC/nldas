...
float new_snow_density(float air_temp) {
  /**************************************************
  ...
  **************************************************/
  ...
  if(air_temp > 0) density_new = (float)NEW_SNOW_DENSITY + 1000.
                     * (air_temp / 100.) * (air_temp / 100.);
  else density_new = (float)NEW_SNOW_DENSITY;
  return (density_new);
}

float snow_albedo(float new_snow,
                   float swq,
                   float cold_content,
                   float dt,
                   int last_snow,
  ...
  /** Aged Snow **/
    ...
    if(cold_content < 0.0 && !MELTING)
      ...
      albedo = NEW_SNOW_ALB*pow(SNOW_ALB_ACCUM_A,
                                pow((float)last_snow * dt / (24. * 3600.0),
                                    SNOW_ALB_ACCUM_B));
#else
      albedo = NEW_SNOW_ALB*pow(SNOW_ALB_ACCUM_A, 
				pow((float)last_snow * dt / 24.,
				    SNOW_ALB_ACCUM_B));
#endif

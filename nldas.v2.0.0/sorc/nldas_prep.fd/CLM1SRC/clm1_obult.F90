...
subroutine clm1_obult (displa,  z0m,      z0h,    z0q,    obu,  &
                      um,      ustar,    temp1,  temp2,  clm1)
...
!=========================================================================
! $Id: clm1_obult.F90,v 1.1.1.1 2003/02/06 16:10:46 jgottsch Exp $
!=========================================================================
...
!=== Local Variables =====================================================
  ...
  real(r8)  zldis,    & ! reference height "minus" zero displacement heght [m]
       clm1_psi,       & ! stability function for unstable case
...
! Temperature profile
  ...
  zldis=clm1%forc_hgt_t-displa
  zeta=zldis/obu
  zetat=0.465
  if (zeta < -zetat) then           ! zeta < -1
     temp1=vkc/(log(-zetat*obu/z0h)-clm1_psi(2,-zetat) &
          + clm1_psi(2,z0h/obu) &
          + 0.8*((zetat)**(-0.333)-(-zeta)**(-0.333)))
  else if (zeta < 0.) then         ! -1 <= zeta < 0
     temp1=vkc/(log(zldis/z0h) - clm1_psi(2,zeta) + &
          clm1_psi(2,z0h/obu))
  else if (zeta <= 1.) then        !  0 <= ztea <= 1
...
! Humidity profile
  ...
  zldis=clm1%forc_hgt_q-displa
  zeta=zldis/obu
  zetat=0.465
  if (zeta < -zetat) then          ! zeta < -1
     temp2=vkc/(log(-zetat*obu/z0q) - &
          clm1_psi(2,-zetat) + clm1_psi(2,z0q/obu) &
          + 0.8*((zetat)**(-0.333)-(-zeta)**(-0.333)))
  else if (zeta < 0.) then         ! -1 <= zeta < 0
     temp2=vkc/(log(zldis/z0q) - &
          clm1_psi(2,zeta)+clm1_psi(2,z0q/obu))
  else if (zeta <= 1.) then        !  0 <= ztea <= 1

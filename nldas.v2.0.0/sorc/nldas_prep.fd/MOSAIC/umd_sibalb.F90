...
subroutine umd_sibalb ( AVISDR, ANIRDR, AVISDF, ANIRDF,        &
                        VLAI, VGRN, ZTH, SNW, ITYP, IRUN, TC, sib )
...
!======== ALATRM = (BLAI + (NLAI - 1) * DLAI - EPSLN)
  ...
  real, dimension(IRUN) :: VLAI, VGRN, ZTH, SNW, TC
...
!=== Added check to make sure total LAI is above 0.15 to avoid model run termination
     IF (ITYP(I) .NE. 12 .AND. VLAI(I) .LT. 0.15) THEN
         VLAI(I) = 0.15
     ENDIF
     ...
     ALPHA = COEFF (sib%ALVDR (1, 1, ITYP (I)), NLAI, LAI ,DX, DY)
     BETA  = COEFF (sib%BTVDR (1, 1, ITYP (I)), NLAI, LAI ,DX, DY)
     GAMMA = COEFF (sib%GMVDR (1, 1, ITYP (I)), NLAI, LAI ,DX, DY)

     AVISDR(I) = ALPHA - ZTH(I)*BETA / (GAMMA+ZTH(I))
     AVISDF(I) = ALPHA-BETA                                &
          + 2.*BETA*GAMMA*(1.-GAMMA*ALOG((1.+GAMMA)/GAMMA))

     ALPHA = COEFF (sib%ALIDR (1, 1, ITYP (I)), NLAI, LAI ,DX, DY)
     BETA  = COEFF (sib%BTIDR (1, 1, ITYP (I)), NLAI, LAI ,DX, DY)
     GAMMA = COEFF (sib%GMIDR (1, 1, ITYP (I)), NLAI, LAI ,DX, DY)

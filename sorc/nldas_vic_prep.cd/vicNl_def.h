...
#define MISSING      -999	/* missing value */
...
typedef struct {
  ...
  double aero_resist;                  /* grid cell mean aerodynamic 
					  resistance  [s/m] */
  double aero_conduct;                  /* grid cell mean aerodynamic 
					  conductance  [m/s] */
  ...
  double liquid[MAX_LAYERS];			/* current liquid moisture in each layer */
  double moist[MAX_LAYERS];				/* current moisture in each layer */
  double moist_total;					/* total soil column moisture */
  double moist_max_total;				/* total soil column max allowable moisture */
  double moist_root;					/* root zone soil moisture */
  double moist_1m;						/* top 1m soil soil moisture */
  ...
  /** Start of changes -- JS **/
  double rainfall;				       /* incoming rainfall (liquid part of precip) */
  /** End of changes -- JS **/
  ...
  double snow_albedo[MAX_BANDS+1];		/* snow albedo (%) */
  double snow_canopy[MAX_BANDS+1];     /* snow captured by canopy (mm) */
  double snow_depth[MAX_BANDS+1];      /* snow depth (cm) */
/** Start of changes -- JS **/
  double snowfall;				       /* incoming snowfall (frozen part of precip): TODO: should this be dependent on snowbands - as it is a function of temperature! */
/** End of changes -- JS **/
  double snow_flux[MAX_BANDS+1];       /* energy flux through the snowpack 
					  [Wm-2] */
  double snow_fluxes[MAX_BANDS+1];      /* energy balance of the snowpack [Wm-2] */
/** Start of changes -- JS **/
  double snow_melt[MAX_BANDS+1];		/* snow melt (mm) */
  double snow_pack_temp[MAX_BANDS+1];	/* snow pack temperature (K) */
  double soil_temp[MAX_LAYERS];			/* soil layer temperature (K) */
/** End of changes -- JS **/
...
/** Start of changes -- JS **/
  double wetness_total;					/* total soil column wetness (%) */
  double wetness_root;					/* root zone wetness (%) */
/** End of changes -- JS **/

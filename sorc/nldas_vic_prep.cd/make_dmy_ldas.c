...
/*********************************************************************** 
   Greg O'Donnell   June, 2000

   Create the day - month - year array.
   Used in the creating of NCEP filenames.
...
/** Start of changes -- JS **/
#if INS2AVE
/* add on an extra hour at the beginning so that we can 
     convert instantaneous data to timestep averaged */
  global->starthour--;
  if(global->starthour < 0) {
    global->starthour = 23;
    global->startday--;
    if(global->startday < 1) {
      global->startmonth--;
      if(global->startmonth < 1) {
        global->startmonth = 12;
        global->startyear--;
      }
      if(LEAPYR(global->startyear)) days[1] = 29;
      global->startday = days[global->startmonth-1];
    }
  }
/*printf("%d %d %d %d\n", global->startyear, global->startmonth, global->startday, global->starthour);*/
#endif
/** End of changes -- JS **/
  ...
  if(global->nrecs < 0) {
    if((global->endyear < 0) || (global->endmonth < 0) 
       || (global->endday < 0) || endhour<0 ) {
          fprintf(stderr,"The model global file MUST define EITHER the number ");
	  fprintf(stderr,"of records to simulate (NRECS), or the year ");
	  fprintf(stderr,"(ENDYEAR), month (ENDMONTH), day (ENDDAY) and ");
	  fprintf(stderr,"hour (ENDHOUR) of the last full simulation day\n");
	  exit(EXIT_FAILURE);
    }

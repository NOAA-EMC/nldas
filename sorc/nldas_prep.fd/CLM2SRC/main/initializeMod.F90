...
!=======================================================================
  ...
  subroutine initialize(eccen       , obliqr      , lambm0    , mvelpp, ldas, tile ,grid    )
...
! 
!-----------------------------------------------------------------------
! $Id: initializeMod.F90,v 1.2 2003/02/21 22:18:24 jgottsch Exp $
!-----------------------------------------------------------------------
    ...
    use time_manager   , only:  get_curr_date, timemgr_init, get_nstep, &
	                        advance_timestep, dtime, start_ymd, start_tod, &
				stop_ymd, stop_tod, get_curr_time
#endif
#if (defined ACCUM)
    use accumulMod     , only : accini
#endif
#if (defined DGVM)
    use accumDGVM      , only : accumDGVMini 
#endif
#if (defined OFFLINE)
...
! ------------------------ local variables -----------------------------
    ...
    real*8   :: temptime
    real     :: tempgmt,tempmnf,ftemphr
    integer  :: tempdoy,nyr,nmth,nday,nhr,nmin,nsec,tempts,itempmn,itemphr
    integer  :: tempyr,tempmo,tempda,temphr,tempmn,tempss,fyr,fmon,fday,fsec
! ----------------------------------------------------------------------
...
!=== LDAS modification: Commented out because we use our own surface datasets
!!    if (fsurdat == ' ') then
!!       call mksrfdat ()
!!    endif
...
!=== LDAS modification: To convert LDAS tilespace variables to CLM tilespace
    call tile2clm2(ldas,tile,grid,wtxy,vegxy)
...
!=== LDAS modification: Passed in ldas and grid modules
    call iniTimeConst (vegxy, ldas, grid)
...
! ----------------------------------------------------------------------
    ...
    if (nsrest > 0) then
     call restrd ()
!===Must reset ldas.crd time step to restart file timestep
     if (ldas%ts /= dtime) then
      print*, "Restart time step not equal ldas.crd time step, must be equal"
      print*, "Time step being reset from ", ldas%ts, " to ", dtime
      ldas%ts = dtime
     endif
    endif
#if (defined DGVM)
    if (nsrest > 0) call iniTimeConstDGVM
#endif
...
!=== LDAS modification: Atmospheric forcing read later, if initialization it is done here
!!    if (masterproc) then
...
!=== Convert model interpolated forcing data from grid to CLM tile space
       call force2tile(ldas,tile,grid)
       call atmdrv(ldas,grid,tile)
     endif
...
! ----------------------------------------------------------------------
    ...
    if(nsrest > 0) then
     ...
     call get_curr_date(fyr, fmon, fday, fsec, offset=-dtime)
     ...
     ftemphr = fsec/3600.0
     temphr = int(ftemphr)
     tempmn = (ftemphr - temphr) * 60
     tempss = 0
     call date2time(temptime,tempdoy,tempgmt,tempyr,tempmo,tempda,temphr,tempmn,tempss)
     ldas%yr   = tempyr
     ldas%mo   = tempmo
     ldas%da = tempda
     ldas%hr = temphr
...
! ----------------------------------------------------------------------
!=== LDAS modification: Vegetation read in later
...
!=== LDAS modification: readini must be false
!!       if (finidat == ' ') then
          readini = .false.
!!       else
!!          readini = .true.
...
!=== LDAS modification: Added ldas and tile modules

       call iniTimeVar (readini, eccen, obliqr, lambm0 , mvelpp, ldas, tile)

...
!=========================================================================
      subroutine clm2_out (ldas, tile, grid)
...
!=== averaging, between output writes
      clm%count=clm%count+1
      
      clm%totfsa=clm%totfsa+clm%fsa
      clm%toteflx_lwrad_net=clm%toteflx_lwrad_net+  
     &            clm%eflx_lwrad_net
      clm%toteflx_lh_tot=clm%toteflx_lh_tot+clm%eflx_lh_tot
      clm%toteflx_sh_tot=clm%toteflx_sh_tot+clm%eflx_sh_tot
      do tt=1,ldas%nch
      clm(tt)%toteflx_soil_grnd=clm(tt)%toteflx_soil_grnd+
     &  clm(tt)%eflx_soil_grnd
	enddo
      clm%totqflx_snomelt=clm%totqflx_snomelt+clm%qflx_snomelt*ldas%ts
      clm%totsolisbd=clm%totsolisbd+
     & (clm%forc_solad(1)+clm%forc_solad(2)+
     &  clm%forc_solai(1)+clm%forc_solai(2))
      clm%totforc_lwrad=clm%totforc_lwrad+clm%forc_lwrad

      clm%totsnow=clm%totsnow+(clm%forc_snow*ldas%ts)
      clm%totrain=clm%totrain+(clm%forc_rain*ldas%ts)
      clm%totqflx_evap=clm%totqflx_evap+(clm%qflx_evap_tot*ldas%ts)

!      clm%totqflx_surf=clm%totqflx_surf+
!     &     (ldas%ts*(clm%qflx_surf+clm%qflx_qrgwl-clm%qflx_qirr))
      clm%totqflx_surf=clm%totqflx_surf+
     &     (ldas%ts*(clm%qflx_surf+clm%qflx_qrgwl))
      clm%totqflx_drain=clm%totqflx_drain+
     &    (ldas%ts*(clm%qflx_drain))

      clm%totqflx_ecanop=clm%totqflx_ecanop+          
     &  (-hvap*(clm%qflx_evap_veg-clm%qflx_tran_veg))
      clm%totqflx_tran_veg=clm%totqflx_tran_veg+clm%qflx_tran_veg*
     &                   (-hvap)
      clm%totqflx_evap_grnd=clm%totqflx_evap_grnd+clm%qflx_evap_grnd*
     &                    (-hvap)
      clm%totqflx_sub_snow=clm%totqflx_sub_snow+clm%qflx_sub_snow*
     &              (-hsub)

!=== Test to see if output writing interval has been reached
      if(mod(ldas%gmt,ldas%writeintc2).eq.0)then
       ldas%numoutc2=ldas%numoutc2+1    !Counts number of output times
...
!=== LDAS Energy Balance Components (time averaged)
       clm%totfsa=(-1)*clm%totfsa/float(clm%count)
       call t2gr(clm%totfsa,g_fsa,ldas%nc,ldas%nr,ldas%nch,
     &  patchvec%wtxy,patchvec%ixy,patchvec%jxy)
       clm%toteflx_lwrad_net=clm%toteflx_lwrad_net/float(clm%count)
       call t2gr(clm%toteflx_lwrad_net,g_eflx_lwrad_net,ldas%nc,
     &  ldas%nr,ldas%nch,patchvec%wtxy,patchvec%ixy,patchvec%jxy)
       clm%toteflx_lh_tot=clm%toteflx_lh_tot/float(clm%count)
       call t2gr(clm%toteflx_lh_tot,g_eflx_lh_tot,ldas%nc,
     &  ldas%nr,ldas%nch,patchvec%wtxy,patchvec%ixy,patchvec%jxy)
	clm%toteflx_sh_tot=clm%toteflx_sh_tot/float(clm%count)
       call t2gr(clm%toteflx_sh_tot,g_eflx_sh_tot,ldas%nc,
     &  ldas%nr,ldas%nch,patchvec%wtxy,patchvec%ixy,patchvec%jxy)
	clm%toteflx_soil_grnd=
     &  (-1)*clm%toteflx_soil_grnd/float(clm%count)
       call t2gr(clm%toteflx_soil_grnd,g_eflx_soil_grnd,ldas%nc,
     &  ldas%nr,ldas%nch,patchvec%wtxy,patchvec%ixy,patchvec%jxy)
       snowflux=(clm%totqflx_snomelt/float(clm%count))*(-hfus)
       call t2gr(snowflux,g_snowflux,ldas%nc,ldas%nr,ldas%nch,
     &  patchvec%wtxy,patchvec%ixy,patchvec%jxy)
	clm%totsolisbd=clm%totsolisbd/float(clm%count)
       call t2gr(clm%totsolisbd,g_solisbd,ldas%nc,ldas%nr,ldas%nch,
     &  patchvec%wtxy,patchvec%ixy,patchvec%jxy)
	clm%totforc_lwrad=clm%totforc_lwrad/float(clm%count)
       call t2gr(clm%totforc_lwrad,g_forc_lwrad,ldas%nc,
     &  ldas%nr,ldas%nch,patchvec%wtxy,patchvec%ixy,patchvec%jxy)

!=== LDAS Water Balance Components (accumulated)
       call t2gr(clm%totsnow,g_snow,ldas%nc,ldas%nr,ldas%nch,
     &  patchvec%wtxy,patchvec%ixy,patchvec%jxy)
       call t2gr(clm%totrain,g_rain,ldas%nc,ldas%nr,ldas%nch,
     &  patchvec%wtxy,patchvec%ixy,patchvec%jxy)
       call t2gr(clm%totqflx_evap,g_qflx_evap,ldas%nc,ldas%nr,ldas%nch,
     &  patchvec%wtxy,patchvec%ixy,patchvec%jxy)
       call t2gr(clm%totqflx_surf,g_qflx_surf,ldas%nc,ldas%nr,ldas%nch,
     &  patchvec%wtxy,patchvec%ixy,patchvec%jxy)
       call t2gr(clm%totqflx_drain,g_qflx_drain,ldas%nc,ldas%nr,
     &  ldas%nch,patchvec%wtxy,patchvec%ixy,patchvec%jxy)
       snowmelt=clm%totqflx_snomelt*ldas%ts
...
!Snow Temperature Calculation
     ...
     &  patchvec%wtxy,patchvec%ixy,patchvec%jxy)
       call t2gr(clm%t_veg,g_t_veg,ldas%nc,ldas%nr,ldas%nch,
     &  patchvec%wtxy,patchvec%ixy,patchvec%jxy)
       call t2gr(clm%t_grnd,g_t_grnd,ldas%nc,ldas%nr,ldas%nch,
     &  patchvec%wtxy,patchvec%ixy,patchvec%jxy)
...
! the snow temperature, bare soil temperature and canopy temperature
     ...
     &  patchvec%wtxy,patchvec%ixy,patchvec%jxy)
       call t2gr(clm%t_rad,g_t_rad,ldas%nc,ldas%nr,ldas%nch,
     &  patchvec%wtxy,patchvec%ixy,patchvec%jxy)
       call t2gr(clm%surfalb,g_surfalb,ldas%nc,ldas%nr,ldas%nch,
     &  patchvec%wtxy,patchvec%ixy,patchvec%jxy)
       call t2gr(clm%h2osno,g_h2osno,ldas%nc,ldas%nr,ldas%nch,
     &  patchvec%wtxy,patchvec%ixy,patchvec%jxy)
       call t2gr(clm%h2ocan,g_h2ocan,ldas%nc,ldas%nr,ldas%nch,
     &  patchvec%wtxy,patchvec%ixy,patchvec%jxy)

!=== LDAS Subsurface State Variables
       do m=1,nlevsoi
        do t=1,ldas%nch
         tempvar(t)=clm(t)%t_soisno(m)
        enddo
...
!=== LDAS Evaporation Components
       ...
       call t2gr(clm%totqflx_ecanop,g_qflx_ecanop,ldas%nc,ldas%nr,
     &  ldas%nch,patchvec%wtxy,patchvec%ixy,patchvec%jxy)
!!!       cantrn=(clm%totqflx_tran_veg/float(clm%count))*(-hvap)
        cantrn=(clm%totqflx_tran_veg/float(clm%count))
       call t2gr(cantrn,g_cantrn,ldas%nc,ldas%nr,ldas%nch,
     &  patchvec%wtxy,patchvec%ixy,patchvec%jxy)
!!!       bare=(clm%totqflx_evap_grnd/float(clm%count))*(-hvap)
       bare=(clm%totqflx_evap_grnd/float(clm%count))
       call t2gr(bare,g_bare,ldas%nc,ldas%nr,ldas%nch,
     &  patchvec%wtxy,patchvec%ixy,patchvec%jxy)
!!!       snowevp=(clm%totqflx_sub_snow/float(clm%count))*(-hsub)
     ...
     &  patchvec%wtxy,patchvec%ixy,patchvec%jxy)
       call t2gr(clm%acond,g_acond,ldas%nc,ldas%nr,ldas%nch,
     &  patchvec%wtxy,patchvec%ixy,patchvec%jxy)
       ccond=ldas%udef
       call t2gr(ccond,g_ccond,ldas%nc,ldas%nr,ldas%nch,
     &  patchvec%wtxy,patchvec%ixy,patchvec%jxy)
       call t2gr(clm%tlai,g_tlai,ldas%nc,ldas%nr,ldas%nch,
     &  patchvec%wtxy,patchvec%ixy,patchvec%jxy)

!=== LDAS Cold Season Processes
       call t2gr(clm%snowdp,g_snowdp,ldas%nc,ldas%nr,ldas%nch,
     &  patchvec%wtxy,patchvec%ixy,patchvec%jxy)
       call t2gr(clm%frac_sno,g_frac_sno,ldas%nc,ldas%nr,ldas%nch,
     &  patchvec%wtxy,patchvec%ixy,patchvec%jxy)
       call t2gr(clm%snoalb,g_snowa,ldas%nc,ldas%nr,ldas%nch,
     &  patchvec%wtxy,patchvec%ixy,patchvec%jxy)

!=== LDAS Forcing Components
       call t2gr(clm%forc_t,g_forc_t,ldas%nc,ldas%nr,ldas%nch,
     &  patchvec%wtxy,patchvec%ixy,patchvec%jxy)
       call t2gr(clm%forc_q,g_forc_q,ldas%nc,ldas%nr,ldas%nch,
     &  patchvec%wtxy,patchvec%ixy,patchvec%jxy)
       call t2gr(clm%forc_u,g_forc_u,ldas%nc,ldas%nr,ldas%nch,
     &  patchvec%wtxy,patchvec%ixy,patchvec%jxy)
       call t2gr(clm%forc_v,g_forc_v,ldas%nc,ldas%nr,ldas%nch,
     &  patchvec%wtxy,patchvec%ixy,patchvec%jxy)
       call t2gr(clm%forc_pbot/100.,g_forc_pbot,ldas%nc,ldas%nr,
     &  ldas%nch,patchvec%wtxy,patchvec%ixy,patchvec%jxy)
...
111    FORMAT(I4)
       ...
       WRITE(90,98,REC=1)'/EXP',LDAS%EXPCODE,'/CLM2/',
...
!=== Open HDF daily output file     
       if((ldas%numoutc2.eq.1.and.ldas%whdf.eq.1))then
        write(90,101,rec=1)'.CLM2grid'
	
        read(90,92,rec=1) (fsubsg(i),i=1,9)

        write(90,92,rec=1)(fbase(i),i=1,c), (fname(i),i=1,29),
     &                    (ftime(i),i=1,8),(fsubsg(i),i=1,9 ) 
...
!=== CLM Output File Name Generation Complete

!=== Set up HDF Input Parameters ===============================================
        prec = 0     !Data precision (0=32 bit, 1=64 bit) 
        timinc= 10000*int(ldas%writeintc2)
...
!=== Write to HDF
     ...
     &     tile(t)%pveg.eq.1)then
           do m=1,nlevsoi
            tempvarts3(tile(t)%col,tile(t)%row,m)=clm(t)%t_soisno(m)
           enddo
          endif
         enddo
...
!=== Write to HDF
     ...
     &     tile(t)%pveg.eq.1)then
           do m=1,nlevsoi 
            tempvarts3(tile(t)%col,tile(t)%row,m)=clm(t)%h2osoi_liq(m)
           enddo
          endif
         enddo
...
!=== End 3D tile space output
         if((24-ldas%gmt).le.ldas%writeintc2.or.
...
!=== Initialize temporary tile array
     ...
     &     float(tile(t)%vegt)
         enddo
     &    ldas%hhmmss,ldas%nc,ldas%nr,kbeg,kount,tempvarts,ldas%rctc)                       
     ...
     &  clm%totfsa,              !Net Surface Shortwave Radiation (W/m2)
     &    ldas,grid,tile) 
         call tshdf(ldas%fidtc,ldas%rctc,vnamet(4),kbeg,kount,
     &  clm%toteflx_lwrad_net,            !Net Surface Longwave Radiation (W/m2)
     &    ldas,grid,tile) 
         call tshdf(ldas%fidtc,ldas%rctc,vnamet(6),kbeg,kount,
     &  clm%toteflx_lh_tot,             !Latent Heat Flux (W/m2)
     &    ldas,grid,tile) 
         call tshdf(ldas%fidtc,ldas%rctc,vnamet(6),kbeg,kount,
     &  clm%toteflx_sh_tot,              !Sensible Heat Flux (W/m2)
     &    ldas,grid,tile) 
         call tshdf(ldas%fidtc,ldas%rctc,vnamet(7),kbeg,kount,
     &  clm%toteflx_soil_grnd,              !Ground Heat Flux (W/m2)
     &    ldas,grid,tile) 
     ...
     &  clm%t_veg,                   !Canopy Temperature (K)
     &    ldas,grid,tile) 
         call tshdf(ldas%fidtc,ldas%rctc,vnamet(19),kbeg,kount,
     &  clm%t_grnd,                  !Bare Soil Temperature (K)
     &    ldas,grid,tile)
     ...
     &  clm%t_rad,                   !Effective Radiative Surface Temperature (K)
     &    ldas,grid,tile) 
         call tshdf(ldas%fidtc,ldas%rctc,vnamet(22),kbeg,kount,
     &  clm%surfalb,                 !Surface Albedo, All Wavelengths (%)
     &    ldas,grid,tile) 
         call tshdf(ldas%fidtc,ldas%rctc,vnamet(23),kbeg,kount,
     &  clm%h2osno,                    !Snowpack Water Equivalent (kg/m2)
     &    ldas,grid,tile) 
         call tshdf(ldas%fidtc,ldas%rctc,vnamet(24),kbeg,kount,
     &  clm%h2ocan,                   !Plant Canopy Surface Water Storage (kg/m2)
     &    ldas,grid,tile) 
     ...
     &  clm%totqflx_ecanop,            !Canopy Surface Water Evaporation (W/m2)
     &    ldas,grid,tile) 
     ...
     &  clm%acond,                  !Aerodynamic Conductance (m/s)
     &    ldas,grid,tile) 
...
!=== Write grid output in HDF format with the subroutine gfio_putvar
     ...
     &   ldas,grid)
         ...
         if((24-ldas%gmt).le.ldas%writeintc2.or.
     &     ldas%endtime.eq.1)then
         ldas%numoutc2=0    !Reset output time counter
        endif
...
!*********************************************
!!!!Changed this call REMEMBER THAT**************
!************************

        IF(LDAS%WGRB.EQ.1)THEN
        CALL GRIBOUTCLM2 (LDAS,GRID,TILE,FBASE,FYRMODIR)
        ENDIF
...
997    format(t36,'Mean',t50,'StDev',t64,'Min',t78,'Max')
       do t=1,ldas%nch
        tempvar(t)=clm(t)%t_soisno(10)
       enddo
       call stats(tempvar,ldas%udef,ldas%nch,vmean,vstdev,vmin,vmax)
       write(60,999)'T_SOISNO10 (K):        ',vmean,vstdev,vmin,vmax
       do t=1,ldas%nch
        tempvar(t)=clm(t)%t_soisno(5)
       enddo
       call stats(tempvar,ldas%udef,ldas%nch,vmean,vstdev,vmin,vmax)
       write(60,999)'T_SOISNO05 (K):        ',vmean,vstdev,vmin,vmax
       do t=1,ldas%nch
        tempvar(t)=clm(t)%t_soisno(1)
       enddo
       call stats(tempvar,ldas%udef,ldas%nch,vmean,vstdev,vmin,vmax)
       write(60,999)'T_SOISNO01 (K):        ',vmean,vstdev,vmin,vmax

       do t=1,ldas%nch
        tempvar(t)=(clm(t)%h2osoi_liq(10))/
     &   (denh2o*clm(t)%dz(10))
       enddo
       call stats(tempvar,ldas%udef,ldas%nch,vmean,vstdev,vmin,vmax)
       write(60,999)'H2OSOI_LIQ10 (Vol):    ',
     &    vmean,vstdev,vmin,vmax
       do t=1,ldas%nch
        tempvar(t)=(clm(t)%h2osoi_liq(5))/
     &   (denh2o*clm(t)%dz(5))
       enddo
       call stats(tempvar,ldas%udef,ldas%nch,vmean,vstdev,vmin,vmax)
     ...
     &    vmean,vstdev,vmin,vmax
       ...
       do t=1,ldas%nch
        tempvar(t)=(clm(t)%h2osoi_ice(10))/
     &   (denice*clm(t)%dz(10))
       enddo
       call stats(tempvar,ldas%udef,ldas%nch,vmean,vstdev,vmin,vmax)
       write(60,999)'H2OSOI_ICE10 (Vol):    ',
     &    vmean,vstdev,vmin,vmax
       do t=1,ldas%nch
        tempvar(t)=(clm(t)%h2osoi_ice(5))/
     &   (denice*clm(t)%dz(5))
       enddo
       call stats(tempvar,ldas%udef,ldas%nch,vmean,vstdev,vmin,vmax)
     ...
     &    vmean,vstdev,vmin,vmax
       do t=1,ldas%nch
        tempvar(t)=(clm(t)%h2osoi_ice(1))/
     &   (denice*clm(t)%dz(1))
       enddo
       call stats(tempvar,ldas%udef,ldas%nch,vmean,vstdev,vmin,vmax)
       write(60,999)'H2OSOI_ICE01 (Vol):    ',
     &    vmean,vstdev,vmin,vmax
       call stats(clm%totfsa,ldas%udef,ldas%nch,vmean,vstdev,vmin,vmax)
       write(60,999)'FSA (W/m2):            ',
     &    vmean,vstdev,vmin,vmax
       call stats(clm%toteflx_lwrad_net,ldas%udef,ldas%nch,
     &    vmean,vstdev,vmin,vmax)
       write(60,999)'EFLX_LWRAD_NET (W/m2): ',
     &    vmean,vstdev,vmin,vmax
       call stats(clm%toteflx_lh_tot,ldas%udef,ldas%nch,
     &    vmean,vstdev,vmin,vmax)
       write(60,999)'EFLX_LH_TOT (W/m2):    ',
     &    vmean,vstdev,vmin,vmax
       call stats(clm%toteflx_sh_tot,ldas%udef,ldas%nch,
     &    vmean,vstdev,vmin,vmax)
       write(60,999)'EFLX_SH_TOT (W/m2):    ',
     &    vmean,vstdev,vmin,vmax
       call stats(clm%toteflx_soil_grnd,ldas%udef,ldas%nch,
     &    vmean,vstdev,vmin,vmax)
     ...
     &    vmean,vstdev,vmin,vmax
       call stats(clm%totsolisbd,ldas%udef,ldas%nch,
     &    vmean,vstdev,vmin,vmax)
       write(60,999)'SOLISBD (W/m2):        ',
     &    vmean,vstdev,vmin,vmax
       call stats(clm%totforc_lwrad,ldas%udef,ldas%nch,
     &    vmean,vstdev,vmin,vmax)
       write(60,999)'FORC_LWRAD (W/m2):     ',
     &    vmean,vstdev,vmin,vmax
       call stats(clm%totsnow,ldas%udef,ldas%nch,vmean,vstdev,vmin,vmax)
       write(60,998)'SNOW (kg/m2):          ',
     &    vmean,vstdev,vmin,vmax
       call stats(clm%totrain,ldas%udef,ldas%nch,vmean,vstdev,vmin,vmax)
       write(60,998)'RAIN (kg/m2):          ',
     &    vmean,vstdev,vmin,vmax
       call stats(clm%totqflx_evap,ldas%udef,ldas%nch,
     &    vmean,vstdev,vmin,vmax)
       write(60,998)'QFLX_EVAP (kg/m2):     ',
     &    vmean,vstdev,vmin,vmax
       call stats(clm%totqflx_surf,ldas%udef,ldas%nch,
     &    vmean,vstdev,vmin,vmax)
       write(60,998)'QFLX_SURF (kg/m2):     ',
     &    vmean,vstdev,vmin,vmax
       call stats(clm%totqflx_drain,ldas%udef,ldas%nch,
     &    vmean,vstdev,vmin,vmax)
     ...
     &    vmean,vstdev,vmin,vmax
       call stats(clm%t_veg,ldas%udef,ldas%nch,vmean,vstdev,vmin,vmax)
       write(60,999)'T_VEG (K):             ',vmean,vstdev,vmin,vmax
       call stats(clm%t_grnd,ldas%udef,ldas%nch,vmean,vstdev,vmin,vmax)
       ...
       call stats(clm%t_rad,ldas%udef,ldas%nch,vmean,vstdev,vmin,vmax)
       write(60,999)'T_RAD (K):             ',vmean,vstdev,vmin,vmax
       call stats(clm%surfalb,ldas%udef,ldas%nch,vmean,vstdev,vmin,vmax)
       write(60,999)'SURFALB (-):           ',vmean,vstdev,vmin,vmax
       call stats(clm%h2ocan,ldas%udef,ldas%nch,vmean,vstdev,vmin,vmax)
       write(60,998)'H2OCAN (kg/m2):        ',vmean,vstdev,vmin,vmax
       call stats(clm%h2osno,ldas%udef,ldas%nch,vmean,vstdev,vmin,vmax)
       ...
       call stats(clm%totqflx_ecanop,ldas%udef,ldas%nch,
     &    vmean,vstdev,vmin,vmax)
     ...
     &    vmean,vstdev,vmin,vmax
       call stats(clm%acond,ldas%udef,ldas%nch,vmean,vstdev,vmin,vmax)
       write(60,998)'ACOND (m/s):           ',
     &    vmean,vstdev,vmin,vmax
       call stats(ccond,ldas%udef,ldas%nch,vmean,vstdev,vmin,vmax)
       write(60,999)'CCOND (m/s):           ',
     &    vmean,vstdev,vmin,vmax
       call stats(clm%tlai,ldas%udef,ldas%nch,vmean,vstdev,vmin,vmax)
       write(60,999)'TLAI (-):              ',
     &    vmean,vstdev,vmin,vmax
       call stats(clm%snowdp,ldas%udef,ldas%nch,vmean,vstdev,vmin,vmax)
       write(60,999)'SNOWDP (m):            ',vmean,vstdev,vmin,vmax
       call stats(clm%frac_sno,ldas%udef,
     &    ldas%nch,vmean,vstdev,vmin,vmax)
       write(60,999)'FRAC_SNO (%):          ',vmean,vstdev,vmin,vmax
       call stats(clm%snoalb,ldas%udef,ldas%nch,vmean,vstdev,vmin,vmax)
...
!=== Reinitialize total arrays to zero
      clm%totfsa=0.                   
      clm%toteflx_lwrad_net=0.
      clm%toteflx_lh_tot=0.
      clm%toteflx_sh_tot=0.
      clm%toteflx_soil_grnd=0.
      clm%totqflx_snomelt=0.
      clm%totsolisbd=0.
      clm%totforc_lwrad=0.
      clm%totsnow=0.
      clm%totrain=0.

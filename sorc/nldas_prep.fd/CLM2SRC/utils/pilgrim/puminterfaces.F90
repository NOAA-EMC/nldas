...
! !INPUT PARAMETERS:
      real(CPP_REAL) input(*)              ! Input array (global)
      type (decomptype) :: decomp          ! Decomposition
      integer incomm                       ! Communicator

! !OUTPUT PARAMETERS:
      real(CPP_REAL) output(*)             ! Output array (local)
...
! !USES:
      use precision
#if defined( SPMD )
      use parutilitiesmodule, only : parpatterntype, parbegintransfer
#endif
      implicit none

! !INPUT PARAMETERS:
      type (parpatterntype) :: pattern     ! Decomposition
      real(CPP_REAL) input(*)              ! Input array
! !OUTPUT PARAMETERS:
      real(CPP_REAL) output(*)             ! Output array
...
#if defined( SPMD )
      call parbegintransfer( pattern, input, output )
#endif
      return
!EOC
...
! !USES:
      use precision
#if defined( SPMD )
      use parutilitiesmodule, only : parpatterntype, parendtransfer
#endif
      implicit none
...
#if defined( SPMD )
      call parendtransfer( pattern, input, output )
#endif
      return
!EOC

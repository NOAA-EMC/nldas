...
subroutine clm1_hydro_snow(clm1)
...
!=========================================================================
! $Id: clm1_hydro_snow.F90,v 1.1.1.1 2003/02/06 16:10:44 jgottsch Exp $
!=========================================================================
...
!=== Local Variables =====================================================
  ...
  real(r8) qin,                    & ! water flow into the elmement (mm/s)
       ...
       vol_liq(clm1%snl+1 : 0),     & ! partial volume of liquid water in layer
       vol_ice(clm1%snl+1 : 0),     & ! partial volume of ice lens in layer
       eff_porosity(clm1%snl+1: 0), & ! effective porosity = porosity - vol_ice
...
!=== End Variable List ===================================================
  ...
  if (clm1%snl+1 >=1) then
     ...
     clm1%qflx_top_soil = clm1%qflx_rain_grnd + clm1%qflx_snomelt
     ...
     wgdif = clm1%h2osoi_ice(clm1%snl+1) + (clm1%qflx_dew_snow - clm1%qflx_sub_snow)*clm1%dtime
     clm1%h2osoi_ice(clm1%snl+1) = wgdif
     if (wgdif < 0.) then
        clm1%h2osoi_ice(clm1%snl+1) = 0.
        clm1%h2osoi_liq(clm1%snl+1) = clm1%h2osoi_liq(clm1%snl+1) + wgdif
     endif
     clm1%h2osoi_liq(clm1%snl+1) = clm1%h2osoi_liq(clm1%snl+1) +  &
          (clm1%qflx_rain_grnd + clm1%qflx_dew_grnd - clm1%qflx_evap_grnd)*clm1%dtime
     clm1%h2osoi_liq(clm1%snl+1) = max(0., clm1%h2osoi_liq(clm1%snl+1))
     
     ! Porosity and partial volume
     
     do j = clm1%snl+1, 0
        vol_ice(j) = min(1., clm1%h2osoi_ice(j)/(clm1%dz(j)*denice))
        eff_porosity(j) = 1. - vol_ice(j)
        vol_liq(j) = min(eff_porosity(j),clm1%h2osoi_liq(j)/(clm1%dz(j)*denh2o))
     enddo
     ...
     qin = 0.
     do j= clm1%snl+1, 0
        clm1%h2osoi_liq(j) = clm1%h2osoi_liq(j) + qin
        if (j <= -1) then
           ! No runoff over snow surface, just ponding on surface
           if (eff_porosity(j)<clm1%wimp .OR. eff_porosity(j+1)<clm1%wimp) then
              qout = 0.
           else
              qout = max(0.,(vol_liq(j)-clm1%ssi*eff_porosity(j))*clm1%dz(j))
              qout = min(qout,(1.-vol_ice(j+1)-vol_liq(j+1))*clm1%dz(j+1))
           endif
        else
           qout = max(0.,(vol_liq(j)-clm1%ssi*eff_porosity(j))*clm1%dz(j))
        endif
        qout = qout*1000.
        clm1%h2osoi_liq(j) = clm1%h2osoi_liq(j) - qout
        qin = qout
     enddo
     
     qout_snowb = qout/clm1%dtime
     clm1%qflx_top_soil = qout_snowb
     
  endif

end subroutine clm1_hydro_snow

...
!=========================================================================
! $Id: clm1_subdiv.F90,v 1.1.1.1 2003/02/06 16:10:47 jgottsch Exp $
!=========================================================================
...
!=== End Variable List ===================================================
  ...
  msno = abs(clm1%snl)
  do k = 1, msno
     dzsno(k) = clm1%dz  (k + clm1%snl)
     swice(k) = clm1%h2osoi_ice(k + clm1%snl)
     swliq(k) = clm1%h2osoi_liq(k + clm1%snl)
     tsno(k)  = clm1%t_soisno (k + clm1%snl)
  enddo
...
! Specified a new snow layer
        ...
        swliq(2) = swliq(1)
        tsno(2)  = tsno(1)
!       write(79,*)'Subdivided Top Node into two layer (1/2)'
     endif
  endif
  ...
  if (msno > 1) then
     if (dzsno(1) > 0.02) then
        ...
        call clm1_combo(dzsno(2),swliq(2),swice(2),tsno(2), &
...
! Subdivided a new layer
  ...
  if (msno > 2) then
     if (dzsno(2) > 0.05) then
        ...
        call clm1_combo(dzsno(3),swliq(3),swice(3),tsno(3), &
...
! Subdivided a new layer
  ...
  if (msno > 3) then
     if (dzsno(3) > 0.11) then
        ...
        call clm1_combo(dzsno(4),swliq(4),swice(4),tsno(4), &
...
! Subdivided a new layer
  ...
  if (msno > 4) then
     if (dzsno(4) > 0.23) then 
        ...
        call clm1_combo(dzsno(5),swliq(5),swice(5),tsno(5), &
  ...
  do k = clm1%snl+1, 0
     clm1%dz(k)   = dzsno(k - clm1%snl)
     clm1%h2osoi_ice(k) = swice(k - clm1%snl)
     clm1%h2osoi_liq(k) = swliq(k - clm1%snl)
     clm1%t_soisno(k)  = tsno (k - clm1%snl)
  enddo

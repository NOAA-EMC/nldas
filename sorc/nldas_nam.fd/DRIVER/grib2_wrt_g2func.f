...
!   21 March 2016: Xia Y.,Revised for NDAS frocing output
!-----------------------------------------------------------------
!
      implicit none
...
!-- set file unit
      ifilw=lugb
!
!---- set up time for 13-23 UTZ forcing -------------------------- 
      if(hour1.ge.13.and.hour1.le.23) then
...
! ---- set up time for 00 UTZ for today --------------------------   
      if (hour1.eq.0) then
      if(iptable(2).eq.8) then
      hour0=23
      read(yesterday,'(i4,i2,i2)') year0,mon0,day0     
...
! ---- set up time for 01-12 UTZ for today -------------------------
      if(hour1.ge.1.and.hour1.le.12) then

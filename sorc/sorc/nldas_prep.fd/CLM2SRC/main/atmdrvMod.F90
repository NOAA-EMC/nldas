...
! 
! Purpose: 
! Read or generate atmospheric grid data
!
! Method: 
! 
...
! 
!-----------------------------------------------------------------------
! $Id: atmdrvMod.F90,v 1.1.1.1 2003/02/06 16:10:50 jgottsch Exp $
!-----------------------------------------------------------------------
...
!=======================================================================
  ...
  subroutine atmdrv(ldas, grid, tile)
...
! ------------------------ local variables ------------------------
    ...
    real(r8) :: temp(begpatch:endpatch) 
    real(r8) :: forc_precc(begpatch:endpatch)
    real(r8) :: forc_precl(begpatch:endpatch)
    real(r8) :: solar(ldas%nch)
    real(r8) :: prcp(ldas%nch)
! -----------------------------------------------------------------

!===LDAS modification: Forcing read in not relevant so commented out  
! -----------------------------------------------------------------
! allocate space for temporaries
! -----------------------------------------------------------------

!    if (.not. allocated(forc_sols )) then
!       allocate (forc_sols(begpatch:endpatch )) ; forc_sols(:) = inf   
!    endif
!    if (.not. allocated(forc_soll )) then
!       allocate (forc_soll(begpatch:endpatch )) ; forc_soll(:) = inf   
!    endif
!    if (.not. allocated(forc_solsd)) then
!       allocate (forc_solsd(begpatch:endpatch)) ; forc_solsd(:) = inf   
!    endif
!    if (.not. allocated(forc_solld)) then
!       allocate (forc_solld(begpatch:endpatch)) ; forc_solld(:) = inf   
!    endif
...
!       ki = begpatch; kf = endpatch
!       call xy2v (lsmlon,lsmlat,forc_txy    ,ki,kf,temp ) ; clm(ki:kf)%forc_t   = temp(ki:kf)
!       call xy2v (lsmlon,lsmlat,forc_uxy    ,ki,kf,temp ) ; clm(ki:kf)%forc_u   = temp(ki:kf)
!       call xy2v (lsmlon,lsmlat,forc_vxy    ,ki,kf,temp ) ; clm(ki:kf)%forc_v   = temp(ki:kf)
!       call xy2v (lsmlon,lsmlat,forc_qxy    ,ki,kf,temp ) ; clm(ki:kf)%forc_q   = temp(ki:kf)
!       call xy2v (lsmlon,lsmlat,zgcmxy      ,ki,kf,temp ) ; clm(ki:kf)%forc_hgt = temp(ki:kf)
!       call xy2v (lsmlon,lsmlat,prcxy       ,ki,kf,temp ) ; forc_precc(ki:kf)   = temp(ki:kf)
!       call xy2v (lsmlon,lsmlat,prlxy       ,ki,kf,temp ) ; forc_precl(ki:kf)   = temp(ki:kf)
!       call xy2v (lsmlon,lsmlat,flwdsxy     ,ki,kf,temp ) ; clm(ki:kf)%forc_lwrad = temp(ki:kf)
!       call xy2v (lsmlon,lsmlat,forc_pbotxy ,ki,kf,temp ) ; clm(ki:kf)%forc_pbot   = temp(ki:kf)
!       call xy2v (lsmlon,lsmlat,forc_psrfxy ,ki,kf,temp ) ; clm(ki:kf)%forc_psrf   = temp(ki:kf)
!       call xy2v (lsmlon,lsmlat,forc_solsxy ,ki,kf,forc_sols ) 
!       call xy2v (lsmlon,lsmlat,forc_sollxy ,ki,kf,forc_soll ) 
!       call xy2v (lsmlon,lsmlat,forc_solsdxy,ki,kf,forc_solsd) 
!       call xy2v (lsmlon,lsmlat,forc_solldxy,ki,kf,forc_solld) 

!$OMP PARALLEL DO PRIVATE (k)
       do k = begpatch, endpatch

!          clm(k)%forc_t        = grid(tile(k)%col,tile(k)%row)%forcing(1)
!	  clm(k)%forc_q        = grid(tile(k)%col,tile(k)%row)%forcing(2)
!	  solar(k)             = grid(tile(k)%col,tile(k)%row)%forcing(3)
!	  clm(k)%forc_solad(1) = solar(k)*35.0/100.0
!	  clm(k)%forc_solad(2) = solar(k)*35.0/100.0
!	  clm(k)%forc_solai(1) = solar(k)*15.0/100.0
!	  clm(k)%forc_solai(2) = solar(k)*15.0/100.0
!	  clm(k)%forc_lwrad    = grid(tile(k)%col,tile(k)%row)%forcing(4)
!	  clm(k)%forc_u        = grid(tile(k)%col,tile(k)%row)%forcing(5)
!	  clm(k)%forc_v        = grid(tile(k)%col,tile(k)%row)%forcing(6)
!	  clm(k)%forc_pbot     = grid(tile(k)%col,tile(k)%row)%forcing(7)
!	  prcp(k)              = grid(tile(k)%col,tile(k)%row)%forcing(8)         

!=== LDAS modification: Equate with LDAS forcing variables instead of NCAR's
         i = patchvec%ixy(k)
	 j = patchvec%jxy(k)
	 
         clm(k)%forc_t        = grid(i,j)%forcing(1)
         clm(k)%forc_q        = grid(i,j)%forcing(2)
         solar(k)             = grid(i,j)%forcing(3)
         clm(k)%forc_solad(1) = solar(k)*35.0/100.0
         clm(k)%forc_solad(2) = solar(k)*35.0/100.0
         clm(k)%forc_solai(1) = solar(k)*15.0/100.0
         clm(k)%forc_solai(2) = solar(k)*15.0/100.0
         clm(k)%forc_lwrad    = grid(i,j)%forcing(4)
         clm(k)%forc_u        = grid(i,j)%forcing(5)
         clm(k)%forc_v        = grid(i,j)%forcing(6)
         clm(k)%forc_pbot     = grid(i,j)%forcing(7)
         prcp(k)              = grid(i,j)%forcing(8)

!=== Used for initialization (startcode=4)
	 if (ldas%startcode .eq. 4 .and. ldas%tscount .eq. 0) then
	   clm(k)%forc_swc1   = grid(i,j)%forcing(13)
	   clm(k)%forc_sdepth = grid(i,j)%forcing(12)
	 endif
	 
!=== LDAS modification: Had to adjust forcing heights in CLM driver.F90 in Biogeophysics 
...
!=== LDAS modification: Modified precip information below to CLM1 version
!          if (forc_precc(k)+forc_precl(k) > 0.) then
!             if (clm(k)%forc_t > (tfrz + tcrit)) then
!                clm(k)%itypprc   = 1
!                clm(k)%forc_rain = forc_precc(k)+forc_precl(k)
!                clm(k)%forc_snow = 0.
!             else
!                clm(k)%itypprc   = 2
!                clm(k)%forc_rain = 0.
!                clm(k)%forc_snow = forc_precc(k)+forc_precl(k)
!             endif
!          else
!             clm(k)%itypprc   = 0
!             clm(k)%forc_rain = 0.
...
!          endif
          ...
          if (prcp(k) > 0.) then
             if (clm(k)%forc_t > (tfrz + tcrit)) then
                clm(k)%itypprc   = 1
                clm(k)%forc_rain = prcp(k)
                clm(k)%forc_snow = 0.
             else
                clm(k)%itypprc   = 2
                clm(k)%forc_rain = 0.
                clm(k)%forc_snow = prcp(k)
             endif
          else
             clm(k)%itypprc   = 0
             clm(k)%forc_rain = 0.
...
!          clm(k)%forc_th  = clm(k)%forc_t * (clm(k)%forc_psrf/clm(k)%forc_pbot)**(rair/cpair)
          clm(k)%forc_th  = clm(k)%forc_t * (clm(k)%forc_pbot/clm(k)%forc_pbot)**(rair/cpair)
...
!===LDAS modification: Done above	  
!          clm(k)%forc_solad(1) = forc_sols(k)     
!          clm(k)%forc_solad(2) = forc_soll(k)    
!          clm(k)%forc_solai(1) = forc_solsd(k)      
!          clm(k)%forc_solai(2) = forc_solld(k)     
...
#if (defined SPMD) 
    call mpi_bcast (atmlon, 1, mpiint, 0, mpicom, ier)
    call mpi_bcast (atmlat, 1, mpiint, 0, mpicom, ier)
#endif
...
#if (defined SPMD) 
    call mpi_bcast (edge_a  , size(edge_a)  , mpir8 , 0, mpicom, ier)
    call mpi_bcast (longxy_a, size(longxy_a), mpir8 , 0, mpicom, ier)
    call mpi_bcast (latixy_a, size(latixy_a), mpir8 , 0, mpicom, ier)
#endif
...
!-----------------------------------------------------------------------
    ...
    use fileutils , only : getfil 
#if (defined SPMD)
    use mpishorthand, only : mpiint, mpicom
#endif
    implicit none
    include 'netcdf.inc' 
...
! ------------------------ arguments ---------------------------------
    ...
    character(len=*), intent(out) :: locfn  !history file to open and read
...
! function declarations
    ...
    tdc(t) = amin1( 50., amax1(-50.,(t-SHR_CONST_TKFRZ)) )

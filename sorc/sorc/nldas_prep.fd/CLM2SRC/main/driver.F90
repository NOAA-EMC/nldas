...
!
!-----------------------------------------------------------------------
! $Id: driver.F90,v 1.1.1.1 2003/02/06 16:10:50 jgottsch Exp $
!-----------------------------------------------------------------------
  ...
  use time_manager  , only : get_step_size, get_curr_calday, get_curr_date, get_nstep, is_last_step
#if (defined RTM)
...
! The latter is done after histhandler in order to
! use the old weights in December's history calculations.
! NB: monp1, dayp1, and secp1 correspond to nstep+1
! ----------------------------------------------------------------------

    if (monp1 == 1 .and. dayp1 == 1 .and. secp1 == dtime) then
...
!!  call histhandler ()
    ...
    if (mcsec_i(1) == 75600) lrestwrt = .true.

...
! 
!-----------------------------------------------------------------------
! $Id: atm_lndMod.F90,v 1.1.1.1 2003/02/06 16:10:49 jgottsch Exp $
!-----------------------------------------------------------------------
...
!$OMP PARALLEL DO PRIVATE(lchnk,ncols,i)
    do lchnk=begchunk,endchunk
       ...
       do i=1,ncols
          ...
          send2d_chunk(i, 2,lchnk)  =  ubot(i,lchnk)  ! Atmospheric state variable m/s
          send2d_chunk(i, 3,lchnk)  =  vbot(i,lchnk)  ! Atmospheric state variable m/s
          send2d_chunk(i, 4,lchnk)  =  thbot(i,lchnk) ! Atmospheric state variable K
          send2d_chunk(i, 5,lchnk)  =  qbot(i,lchnk)  ! Atmospheric state variable kg/kg
          ...
          send2d_chunk(i, 9,lchnk)  =  precsc(i,lchnk)*1000.                  !convert from m/sec to mm/sec
          send2d_chunk(i,10,lchnk)  =  precsl(i,lchnk)*1000.                  !convert from m/sec to mm/sec
          send2d_chunk(i,11,lchnk)  =  (precc(i,lchnk) - precsc(i,lchnk))*1000. !convert from m/sec to mm/sec
          send2d_chunk(i,12,lchnk)  =  (precl(i,lchnk) - precsl(i,lchnk))*1000. !convert from m/sec to mm/sec

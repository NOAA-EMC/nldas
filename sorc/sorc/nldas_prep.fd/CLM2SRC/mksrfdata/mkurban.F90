...
! 
!-----------------------------------------------------------------------
! $Id: mkurban.F90,v 1.1.1.1 2003/02/06 16:10:48 jgottsch Exp $
!-----------------------------------------------------------------------

  use precision
  use clm_varpar    !lsm parameters
  use clm_varsur    !lsm surface variables
...
! Obtain input grid info
  ...
  if (ier/=0) call endrun
  allocate (urb_i(nlon_i,nlat_i), stat=ier)     
  if (ier/=0) call endrun
...
! Obtain input data
  ...
  call wrap_get_var_realx (ncid, varid, urb_i)
...
! Error checks
        ...
        if (urb_o(io,jo) > 100.000001) then

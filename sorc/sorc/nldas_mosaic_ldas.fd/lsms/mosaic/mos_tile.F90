...
!=========================================================================
         ...
         SUBROUTINE MOSTILE (RESET, &
         	  ...
         	  VGSLOX,VGPH1X,VGPH2X,VGRPLX,&
          	  ...
          	  VGRDRS,VGZ2,VGROTD,&
          	  VGDFAC,VGTLL,VGTU,&
                 VGTCF1,VGTCF2,VGTCF3,tempcc,tempac,m)
...
!CCC *
      ...
      INTEGER   FRSTCH, MemFac,ISINDX,m
...
!line below is good value
      DATA CSOIL0 /70000./
!line below is bad value used in JGR simulations
!      DATA CSOIL0 /175000./
...
!ccc	TILE%CC=lis%d%udef
          tempcc=-9999.
	endif
        tempac=CD*UM
!        print*, "IN MOS_TILE"
!        print*, tempac, tempcc, rc
      RETURN
      END
!CCC *
!CCC * [ END CHIP ]
...
!CCC *
      INTEGER   FRSTCH
      INTEGER  NLAY, SFCLY, ROOTLY, RECHLY,iii
...
!          print*, "mflux1.1 ",TC, Q0, A12, DEA, DTC,SNOW
          DTC = ( Q0 - A12*DEA ) / A11
...
!CCC *
! Brian Cosgrove commented back in the two lines below
      VPDSTR = 1. - (ESATTC-EA) * VGDFAC
      VPDSTR  = AMIN1( 1., AMAX1( VPDSTR, 1.E-10 ) )
!Brian Cosgrove commented out the line below
!this line was used in the first round of official JGR NLDAS runs
!      VPDSTR = 1.
!CCC *
 100  CONTINUE
!CCC *
...
!     fix for overly large values of RC
      IF(RC .GT. 1E26 ) RC = 1E26
...
!CCC * (Note: ETCRIT arbitrarily set to ~-5 W/m2, or -2.e-6 mm/sec.)
      DATA ETCRIT/ -2.E-6 /
!CCC *
!CCC * -----------------------------------------------------------------

...
!</debug>
  ...
  type clm1d
     ...
     integer  :: imelt(-4:10)       !flag for melting (=1), freezing (=2), Not=0 (new)        
     integer  :: count
...
!     real(r8) :: londeg         !longitude for subgrid patches (degrees)
     ...
     real(r8) :: zi(-5:10) !interface level below a "z" level (m)
     real(r8) :: dz(-4:10) !layer depth (m)
     real(r8) :: z (-4:10) !layer thickness (m)
     real(r8) :: bsw   (10)       !Clapp and Hornberger "b"
     real(r8) :: watsat(10)       !volumetric soil water at saturation (porosity)
     real(r8) :: hksat (10)       !hydraulic conductivity at saturation (mm H2O /s)
     real(r8) :: sucsat(10)       !minimum soil suction (mm)
     real(r8) :: csol  (10)       !heat capacity, soil solids (J/m**3/Kelvin)
     real(r8) :: tkmg  (10)       !thermal conductivity, soil minerals  [W/m-K]  (new)
     real(r8) :: tkdry (10)       !thermal conductivity, dry soil       (W/m/Kelvin)
     real(r8) :: tksatu(10)       !thermal conductivity, saturated soil [W/m-K]  (new)
     real(r8) :: rootfr(10)       !fraction of roots in each soil layer
     real(r8) :: rootr(10)        !effective fraction of roots in each soil layer
...
!     real(r8) :: forc_psrf           !surface pressure (Pa)
     real(r8) :: forc_solad(2)  !direct beam radiation (vis=forc_sols , nir=forc_soll )
     real(r8) :: forc_solai(2)  !diffuse radiation     (vis=forc_solsd, nir=forc_solld)
!clm+
...
! Temperatures
     ...
     real(r8) :: t_soisno(-4:10) !soil temperature (Kelvin)
...
!     real(r8) :: qflx_snow_grnd                  !snow on ground after interception (mm H2O/s) [+]
     ...
     real(r8) :: h2osoi_liq(-4:10)  !liquid water (kg/m2) (new)
     real(r8) :: h2osoi_ice(-4:10)  !ice lens (kg/m2) (new)
     real(r8) :: h2osoi_vol(10)             !volumetric soil water (0<=h2osoi_vol<=watsat) [m3/m3]
...
!     real(r8) :: snowliq                         !average snow liquid water
     ...
     real(r8) :: frac_iceold(-4:10) !fraction of ice relative to the total water (new)
!     real(r8) :: rsw                             !soil water content for root zone
     real(r8) :: eff_porosity(10)           !effective porosity = porosity - vol_ice
...
!     real(r8) :: albi(2)   !surface albedo (diffuse)                    
     real(r8) :: albgrd(2) !ground albedo (direct)                      
     real(r8) :: albgri(2) !ground albedo (diffuse)                     
     real(r8) :: fabd(2)   !flux absorbed by veg per unit direct flux   
     real(r8) :: fabi(2)   !flux absorbed by veg per unit diffuse flux  
     real(r8) :: ftdd(2)   !down direct flux below veg per unit dir flx 
     real(r8) :: ftid(2)   !down diffuse flux below veg per unit dir flx
     real(r8) :: ftii(2)   !down diffuse flux below veg per unit dif flx
     real(r8) :: fsun           !sunlit fraction of canopy
     real(r8) :: surfalb        !instantaneous all-wave surface albedo
     real(r8) :: snoalb         !instantaneous all-wave snow albedo
...
!*************************************************************************
     ...
     real(r8) :: rhol(2)   ! pft_varcon leaf reflectance  : 1=vis, 2=nir 
     real(r8) :: rhos(2)   ! pft_varcon stem reflectance  : 1=vis, 2=nir 
     real(r8) :: taul(2)   ! pft_varcon leaf transmittance: 1=vis, 2=nir 
     real(r8) :: taus(2)   ! pft_varcon stem transmittance: 1=vis, 2=nir 
...
! -----------------------------------------------------------------
     ...
     real(r8) ::  totfsa              ! solar absorbed solar radiation [W/m2]
     real(r8) ::  toteflx_lwrad_net   ! net longwave radiation [W/m2]
     real(r8) ::  toteflx_lh_tot      ! total latent heat flux [W/m2]
     real(r8) ::  toteflx_sh_tot      ! total sensible heat flux [W/m2]
     real(r8) ::  toteflx_soil_grnd   ! ground heat flux [W/m2]
     real(r8) ::  totqflx_snomelt     ! snowmelt heat flux [W/m2]
...
!     real(r8) ::  totforc_lwrad       ! atmospheric infrared (longwave) radiation [W/m2]
     ...
     real(r8) ::  totqflx_surf        ! surface runoff [mm]
     real(r8) ::  totqflx_drain       ! subsurface runoff [mm]
     ...
     real(r8) ::  acond
     real(r8) ::  canopint
!new additions for alma
     real(r8) ::  soilmtc_prev      ! Total column soil moisture for the prev.timestep
     real(r8) ::  h2osno_prev       ! Total column snow water equivalent for the prev.timestep

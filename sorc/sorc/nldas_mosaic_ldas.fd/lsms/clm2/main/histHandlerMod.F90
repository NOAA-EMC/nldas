...
! Variables
  ...
  logical, public  :: lrestwrt           !true => write restart file at this time step 
...
!   o close history file and dispose to mass store (only if file is open)
!   o reset time sample counter to zero if file is full
!
...
! 
!-----------------------------------------------------------------------
! $Id: histHandlerMod.F90,v 1.7 2004/11/24 23:24:17 jim Exp $
!-----------------------------------------------------------------------
...
!       print*, "IN HISTHAND: before ehi(i) if ", m, nbeghis(m), ehi(m), ntim(m) 
...
!         print*, "IN HISTHAND: in ehi(i) if ", m, nbeghis(m), ehi(m), ntim(m)
...
!    print*, "IN HISTHAND: after ehi(i) if ", m, nbeghis(m), ehi(m), ntim(m)

    call close_and_disp (lhisdisp, lstop, lremovh)
    
!    print*, "AFTER CLOSE/DISP ", lhisdisp, lstop, lremovh
...
! reset number of time samples to zero if file is full
       ...
       end if  ! end of if-lhisdisp block
...
!       print*, "CURR TIME/DATE ", yr, mon, day, mcsec, mdcur, mscur
!       print*, "IN HISTEND ", m, nstep, ehi(m), hist_nhtfrq(m), nbeghis(m)
...
! Sets values for lhisdisp, lstop, lremovh (arguments) 
! and lrestwrt (module variable)
!
! Method: 
! o if history file is full (ntim = mfilt) or 
! o if have reaced end of run as specified by either cam or the
!   flux coupler 
! write-dispose restart file:
...
!   o when history file is closed and disposed
...
#if (defined COUP_CSM)
    use clm_csmMod          !csm routines and variables
#endif
...
! -----------------------------------------------------------------
 ...
 print*, "IN CLOSE/DISP ", ntim(1), lrestwrt, lhisdisp, lstop, lremovh
...
!    print*, "IN HIST HANDLE ", yr, mon, day, mcsec, ntim(1), hist_mfilt(1)
!    print*, lrestwrt

     print*, "IN CLOSE/DISP before ntime if/endif ", ntim(1), lrestwrt, lhisdisp, lstop, lremovh
...
!!     if (mcsec == 75600)  then
       ...
       lhisdisp(:) = .true.
       lremovh(1) = .true.
       print*, "IN CLOSE/DISP in ntime if/endif ", ntim(1), lrestwrt, lhisdisp, lstop, lremovh
       do m = 2,nhist
          if (ntim(m)==hist_mfilt(m)) then
             lremovh(m) = .true.
          else

...
! 
!-----------------------------------------------------------------------
! $Id: histUpdate.F90,v 1.6 2004/11/24 22:57:16 jim Exp $
!-----------------------------------------------------------------------
...
! atmospheric input fields
  print*, 'not supposed to be in histupdate..'
#if 0 
...
!$OMP PARALLEL DO PRIVATE (K)
  ...
  call histslf ('TG      ', clm(begpatch:endpatch)%t_grnd)  
...
!     tmpslev(k) = 0._r4
!     do i = 1, ndst
!        tmpslev(k) = tmpslev(k) + clm(k)%flx_mss_vrt_dst(i) 
!     end do
!  end do
...
!     tmpslev(k) = 0._r4
!     do i = 1, ndst
!        tmpslev(k) = tmpslev(k) + clm(k)%vocflx(i) 
!     end do
!  end do

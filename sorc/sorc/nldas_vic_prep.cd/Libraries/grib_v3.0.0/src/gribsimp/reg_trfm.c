 ...
 */
#if PROTOTYPE_NEEDED
void xy_index(REG_GEOM *geom, long *ix, long *iy, long *index, int *status)
#else
 ...
 */
#if PROTOTYPE_NEEDED
void index_xy(REG_GEOM *geom, long *index, long *ix, long *iy, int *status)
#else
 ...
 */
#if PROTOTYPE_NEEDED
void latlon_xy(REG_GEOM *geom, double *lat, double *lon, double *x, double *y, 
		int *status)
#else
...
#endif
{
    ...
    double mid_pt;              /* middle of geom in columns */
...
/*      Take care of longitude wrap-around  */
	mid_pt = ((double)geom->nx + 1.) / 2.0;
        if (*x < 1.) {
            temp1 = (diff_lon + 360.) / (double)geom->parm_2 +
		    (double)geom->orig_ix;
            if (fabs(mid_pt - temp1) < fabs(mid_pt - *x))
		*x = temp1;
        }
	else if (*x > (double)geom->nx) {
            temp1 = (diff_lon - 360.) / (double)geom->parm_2 +
		    (double)geom->orig_ix;
            if (fabs(mid_pt - temp1) < fabs(mid_pt - *x))
		*x = temp1;
	}
        *y    = (geom->lat - *lat) / (double)geom->parm_1 +
		(double)geom->orig_iy;
...
/*      Check lat is not equal to 90. or -90. (singular points) */
        ...
        if (geom->parm_1 == geom->parm_2) {
            temp1 = sign * sin((double)geom->parm_1 * PI180);
        } else {
 ...
 */
#if PROTOTYPE_NEEDED
void xy_latlon(REG_GEOM *geom, double *x, double *y, double *lat, double *lon, 
		int *status)
#else
 ...
 */
#if PROTOTYPE_NEEDED
void index_latlon(REG_GEOM *geom, long *index, double *lat, double *lon, 
		  int * status)
#else
 ...
 */
#if PROTOTYPE_NEEDED
void latlon_index( REG_GEOM *geom, double   *lat, double   *lon,
		   long *index, int  *status)
#else
 ...
 */
#if PROTOTYPE_NEEDED
void getf_latlon( REG_GEOM *geom, double   *lat, double   *lon,
		  float    *fbuff, float    *fpixel, int      *status)
 ...
 */
#if PROTOTYPE_NEEDED
void putf_latlon( REG_GEOM *geom, double   *lat, double   *lon,
		float *fpixel, float *fbuff, int *status)
#else
 ...
 */
#if PROTOTYPE_NEEDED
void get_int_dis( REG_GEOM *geom, double   *x, double   *y, double   *lat,
...
/*  Check y value different from y of origin */
    if (fabs(*y - (double)geom->orig_iy) < 1.e-5) {
        fprintf(stderr, "Error in get_int_dis;  y: %lf  equals  orig_iy: %lf\n",
                *y, geom->orig_iy);
        *status = -1;
        return;
    }
...
/*  Check x value different from x of origin */
    if (fabs(*x - (double)geom->orig_ix) < 1.e-5) {
        fprintf(stderr, "Error in get_int_dis;  x: %lf  equals  orig_ix: %lf\n",
                *x, geom->orig_ix);
        *status = -1;
        return;
    }
 ...
 */
#if PROTOTYPE_NEEDED
void grid_map( REG_GEOM  *geom_a, REG_GEOM  *geom_b,
		float *fbuff_a, float *fbuff_b, int *status)

...
!EOP      
  ...
  call lis_log_msg('MSG: read_gswp_w_bpower -- Reading GSWP w_bpower file: '// &
                   trim(lis%p%w_bpower_file))

  status = nf90_open(path=lis%p%w_bpower_file, mode=nf90_nowrite, ncid=ncid)
  status = nf90_inq_varid(ncid, "W_bpower", varid)
...
end subroutine read_gswp_w_bpower

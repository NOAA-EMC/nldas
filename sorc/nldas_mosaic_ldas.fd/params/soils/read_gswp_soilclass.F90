...
!EOP      
  ...
  call lis_log_msg('MSG: read_gswp_soilclass -- '  // &
                   'Reading GSWP soil class file: '// &
                   trim(lis%p%soilclass_file))

  status = nf90_open(path=lis%p%soilclass_file, mode=nf90_nowrite, ncid=ncid)
  status = nf90_inq_varid(ncid, "SoilClass", varid)
...
end subroutine read_gswp_soilclass

...
#if ( defined USE_NETCDF )
  call lis_log_msg('MSG: read_gswpclay -- Reading GSWP clay file')

  allocate(clay(lis%d%glbnch))
  
  call lis_log_msg('MSG: read_gswpclay -- Reading clay file: '// &
                   trim(lis%p%clfile))

  status = nf90_open(path=lis%p%clfile, mode=nf90_nowrite, ncid=ncid)
  status = nf90_inq_varid(ncid, "Clay", clayid)
  status = nf90_get_var(ncid, clayid, clay)
  ...
  do c=1,lis%d%lnc
     do r=1,lis%d%lnr
        ...
        if ( gindex(cindex,rindex) /= -1 ) then 
           array(cindex,rindex) = clay(gindex(cindex,rindex))
        endif
     enddo
  enddo
!EOC

...
!EOP      
  ...
  call lis_log_msg('MSG: read_gswp_w_sat -- Reading GSWP w_sat file: '// &
                   trim(lis%p%w_sat_file))

  status = nf90_open(path=lis%p%w_sat_file, mode=nf90_nowrite, ncid=ncid)
  status = nf90_inq_varid(ncid, "W_sat", varid)
...
end subroutine read_gswp_w_sat

      subroutine grib2_wrt_g2func(bmap,hour1,writeint,beforeyester,
     &           yesterday,fld,iptable,lugb,ierr)
!-----------------------------------------------------------------
! ABSTRACT: This routine is to write out a new grib2 file
!   March 2013:     J.Wang
!   23 September 2013: Revised by Youlong Xia for NLDAS output
!   11 July 2016: Revised by Youlong Xia (change Order of spatial difference
!                 from 2 to 1 to aoid grib2 output issue   
!-----------------------------------------------------------------
!
      implicit none
...
!
      ...
      real(4) writeint      
!
!      character(255) :: gribfile
      character*1 cgrib(max_bytes)
      character*8 beforeyester,yesterday
...
!-----------------------------------------------------------------
       ...
       hour0=hour1-writeint
 
       if(hour0.lt.0) then 
       hour0=24-writeint
       read(beforeyester,'(i4,i2,i2)') year0,mon0,day0
       read(yesterday,'(i4,i2,i2)') year,mon,day
       else
...
!-- Open GRIB2 file
!----open GRIB2 file in mos_almaout.F90 in /src/lsms/mosaic and pass
!---------    ifilw to this subroutine ---------------------------- 
      cgrib=''
!

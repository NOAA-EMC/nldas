...
!-------------------------------------------------------------------------
!BOP
! !ROUTINE: getnldas2.F90
!
! !DESCRIPTION:
!  Opens, reads, and interpolates NCEP-LDAS forcing (NARR).  
!
...
!=== Determine Required NCEP Data Times (The previous hour and the future hour)
  ...
  if(timenow.gt.nldas2drv%nldas2time2) then 
     movetime = 1
     lis%f%findtime2 = 1
  endif
  ...
  if(lis%t%tscount.eq.1 .or.lis%f%rstflag.eq.1  ) then    !beginning of the run	
   ...
   if(movetime.eq.1) then
	print *,'moving time2 into time1'
      nldas2drv%nldas2time1=nldas2drv%nldas2time2
      do f=1,lis%f%nf
         do c=1,lis%d%ngrid
...
!=== the following looks back 10 days, at the same hour to fill data gaps.
      ...
      do 
         if ( ferror /= 0 ) exit
         try=try+1
         call ncepfile2(name,nldas2drv%nldas2dir,yr1,mo1,da1,hr1)
         !print *,'yr1,mo1,da1,hr1',yr1,mo1,da1,hr1
         print *,'getting file1.. ',name
         call retnldas2(1,name,ferror,1,0)
         if(ferror.eq.1) nldas2drv%nldas2time1=time1
         call tick(time1,doy1,gmt1,yr1,mo1,da1,hr1,mn1,ss1,ts1)
         if(try.gt.11)then
...
!=== the following looks back 10 days, at the same hour to fill data gaps.
      ...
      do 
         ...
         try=try+1
         !print *,'1 yr2,mo2,da2,hr2',yr2,mo2,da2,hr2
         call ncepfile2(name,nldas2drv%nldas2dir,yr2,mo2,da2,hr2)
	 !print *,'2 yr2,mo2,da2,hr2',yr2,mo2,da2,hr2
         print *,'getting file2.. ',name
         call retnldas2(2,name,ferror,1,0)
         !print *,'3 time2,yr2,mo2,da2,hr2',time2,yr2,mo2,da2,hr2,'ferror=',ferror
         if(ferror.eq.1)nldas2drv%nldas2time2=time2
...
! !INTERFACE:
 subroutine ncepfile2(name,ncepdir,yr,mo,da,hr)
!EOP
   implicit none
...
!   write(90,97,rec=1)'.lsmforce_noaa'
   write(90,97,rec=1)'.nldasforce-a.grb'

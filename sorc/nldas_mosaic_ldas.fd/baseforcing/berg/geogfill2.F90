...
!
! !DESCRIPTION:
!  Fill in grid points that are invalid in LIS due to differences in
...
!EOP  
  ...
  integer :: nc,nr                       ! dimensions and forcing mask
  integer :: smask(nc,nr)                ! grid points where ldas
  real    :: fdata(nc,nr)                    ! interpolated forcing field
              ...
              do ii = i-5, i+5
                 do jj = j-5, j+5
                    if ( (v.ne.3) .and. (v.lt.8) ) then
                       ...
                       if (ii.gt.0.and.jj.gt.0.and.smask(ii,jj).eq.0) then
                          ssum = ssum + fdata(ii,jj)
                          scnt = scnt + 1
                       endif
                    endif
...
!              if(v.eq.3.and.i.eq.11.and.j.eq.3) print*, i,j,savg,fdata(i,j)
              fdata(i,j) = savg
           endif
        endif
     enddo

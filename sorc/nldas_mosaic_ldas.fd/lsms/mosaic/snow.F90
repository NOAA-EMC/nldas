...
!-------------------------------------------------------------------------
      ...
      subroutine snowrt( &
                       ts,t1,area,pr,snowf,dtsi,eturb,dedtc,dedea, &
                       dhsdtc,dhsdea,esattc,desdtc,dhlwtc,ea,&
                       swnet,hlwdwn,hlwtc,hsturb,&
                       xklhw,xklh,zc,bug,&
                       wesn,htsnn,sndz,&
                       tpsn,areasc,pre,fh31w,fh31i,fh31d,hlwout, &
                       evap0,shflux,areasc0 &
                      )
      parameter (ngm=7)
...
!      call cpu_time(tstart)
      ...
      if (snowd .ge. xsmin)then
         ...
         dtm=1200
         DO I=1,3
            dtm2=CPSN(I)*(SNDZ(I)*SNDZ(I))/(TKSN(I)+1E-08)
            DTM=MIN(DTM,dtm2)
         enddo
         if (dtm .lt. 1200)then
            xsmin=snowd
...
!1231         htsnn(1)=-fsn*wesn(1)+tpsns*cpsn(1)*sndz(1)
...
!**** if the water equivalent snow depth is less than xsmin *****
!******************we use a single layer snow model***************
!
      if (snowd .gt. 0. .and. snowd .le. xsmin)then
!**** lets initialize all of our snow parameters*****
!1/6/98         sndz(1)=(sndz(1)+sndz(2)+sndz(3))
...
!******* from the layer heat and water contents, calculate old layer
!        temperatures
         tpsn(1)=0
   ...
   9     continue
!
!******* calculate the heat flow in each layer and determine surface
!        energy fluxes
         factw=area(1)
         facti=area(2)
         factd=area(3)
...
! * test
!         fh1=fh0
!         fhsn(1)=fbsw+fblw-evp-sen+fh1
!         evap=evp/alhx
! * end of test
!1/5/98         evap=(evap/1000.)*dts
         evap0=evap*alhx
         evap=areasc*(evap/1000.)*dts
!**** take care of evporation/condensation from the snow surface
         if(evap .ge. 0.)then
...
!1231         htsnn(1)=htsnn(1)+evap*fsn
         pre=0
         smelt=0
         tmelt=0
         flowth=0
         fresh=0
!******* take care of meltwater moving through the pack
         wcont(1)=(1-fices(1))*wesn(1)
         smelt=wcont(1)
         wesn(1)=wesn(1)-smelt
         pre=smelt/dts
!030199         htsnn(1)=htsnn(1)+smelt*fsn
!******* take care of water flow through due to precip
         if(effpr-snowf .gt. 0)pre=pre+areasc*(effpr-snowf)
!******* update snowdepth(water equiv.) for snowf
         fresh=snowf*dts
!******* scale the fluxes by the snow covered fraction
!1231         fh31w=fh31w*areasc
!1231         fh31i=fh31i*areasc
!1231         fh31d=fh31d*areasc
!1231         hlwout=hlwout*areasc
!1231         evap=evap*areasc
!1231         shflux=shflux*areasc
!******* update our new boundaries, heat and water contents
         if (iflag .eq. 2) then
            wesn(1)=wesn(1)+fresh
            cpnew=150.*1.9e6/920
...
!
!******* from surface-atm flux and snow-ground flux determine the
!        snow layer heat content
!1231         htsnn(1)=htsnn(1)+(fhsn(1))*dts
         htsnn(1)=htsnn(1)+((fhsn(1))*dts)*areasc
!******* from updated heat and water content, determine fraction of ice
!        and temp of snow layers
         tpsn(1)=0
         fices(1)=-htsnn(1)/(fsn*wesn(1))
...
!******* determine the terrestrial temperature
...
!2/22/99            htsnn(i)=-fsn*wesn(i)+tpsn(1)*cpsn(i)*
!2/22/99     1                (wesn(i)*1000./rhos(1))
...
!************* end of single layer snow model************************
!
!**** if the water equivalent snow depth is greater than xsmin*******
!************************we use a three layer snow model*************
!
...
!            htsno(i)=htsnn(i)
         enddo
!******* from layer heat and water content determine fraction of ice and
...
!******* calculate the ground-snow energy flux
...
!******* calculate the heat flux between snow layers
         fhsn(3)=-sqrt(tksn(2)*tksn(3))*(tpsn(2)-tpsn(3))/ &
        (sdnew(1)/2-sdnew(3)/2)
!******* from the energy fluxes calculate layer heat content
...
!030199               htsnn(i)=htsnn(i)+smelt*fsn
!
...
!******* update water equivalent snowdepth from snowf
         fresh=snowf*dts
!******* update the snow density due to compaction. taken from verseghy
!         international journal of climatology,vol 11, 111-133,1991
         do 95 i=1,3
            rhos(i)=wesn(i)*1000./sndz(i)
           ...
           (4000./tplk)-.02*rhos(i)))*dts
            sndz(i)=wesn(i)*1000./rhos(i)
   95    continue
!******* update our new boundaries
...
!******* since the snow boundary is a moving boundary the heat contents
!        and water equivalents of the snow layers must be updated.
!        From old layer heat and mass contents and old boundaries
!        redistribute new heatand water contents to the new layers.
!******* dtermine the heat capacity, heat content, and water content of
!        fresh snow
         cpnew=150.*1.9e6/920
         hnewp=(ts-tfrz)*cpnew-fsn*150./1000.
         wfrsh=150./1000.
!******* determine heat and water contents of the old layer distribution
...
!******* calculate the heat contents and water equiv for the new layer 3
        ...
        'bad dz layer 3',sndz(2),dz1,dz2,dz3,zfsh
!******* calculate the heat contents and water equiv for the new layer 2
...
!******* calculate the heat contents and water equiv for the new layer 1
        ...
        'bad dz layer 1',sndz(1),dz1,dz2,dz3,zfsh
!******* determine layer density(rhos(i)), specific heat
!        content(cpsn(i)) and thermal conductivity(tksn(i))
         do 100 i=1,3
            rhos(i)=wesn(i)*1000/sndz(i)
            cpsn(i)=rhos(i)*1.9e6/920
            tksn(i)=3.2217e-12*(rhos(i)*1e3)*(rhos(i)*1e3)
...
!              stop 'rhos'
 ...
 100     continue
         totsn=sndz(1)+sndz(2)+sndz(3)
         snowd=wesn(1)+wesn(2)+wesn(3)
!******* from the new layer heat and water contents, calculate layer
!        temperatures
         do 115 i=1,3
            fold(i)=fices(i)
            tpsn(i)=0

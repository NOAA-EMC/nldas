...
!=======================================================================
  ...
  subroutine control_init (lis, cam_caseid , cam_ctitle, cam_irad , cam_nsrest, &
...
! 
!-----------------------------------------------------------------------
! $Id: controlMod.F90,v 1.13 2004/11/24 23:24:17 jim Exp $
!-----------------------------------------------------------------------
  use clm_varctl
...
!    print*, "TIMING = ", start_ymd, start_tod, stop_ymd, stop_tod, dtime
...
! control variables
    ...
    write(cecode,'(i3.3)') lis%o%expcode
    caseid  = cecode
    ctitle  = ' '
    caseoutdir = lis%o%odir
!===Set to assure CLM2 internal restart routines not called, may need to be used later
    if (lis%o%startcode == 3) nsrest = 0
    if (lis%o%startcode == 2) nsrest = 0
    if (lis%o%startcode == 1) nsrest = 1    
...
!===Do not want to read in namelist parameters from standard input, set in lis.crd
...
#endif
     !rpntdir = './'
     rpntdir = ''  ! full path is built into clm2_rfile
     rpntfil = clmdrv%clm2_rfile

...
!    use clm_varpar  , only : lsmlon, lsmlat
    ...
    use clm_varmap  , only : patchvec,numpatch
    use fileutils   , only : getfil
    
    use lisdrv_module, only : lis    ! LDAS variables
!    use grid_module    ! Grid variables
    
    implicit none
    
!    type (lisdec)  :: lis
!    type (griddec)  :: grid(lis%d%nch)
!    type (griddec)  :: grid(di_array(iam))
!    include 'netcdf.inc'
...
!    real(r8) :: forc_precl(begpatch:endpatch)
    ...
    real :: forcing(10)
! -----------------------------------------------------------------
!      if(lis%f%findtime1==1 .or. lis%f%findtime2==1) then
!===LDAS modification: Forcing read in not relevant so commented out  
!    do k=1,di_array(iam)
!         i = patchvec%ixy(k)
!         j = patchvec%jxy(k)
!       if(k==54) then
!       print*,iam,k,grid(k)%forcing(1),grid(k)%forcing(4) 
!       endif
         clm(k)%forc_t        = forcing(1)
         clm(k)%forc_q        = forcing(2)
         solar                = forcing(3)
         clm(k)%forc_solad(1) = solar*35.0/100.0
         clm(k)%forc_solad(2) = solar*35.0/100.0
         clm(k)%forc_solai(1) = solar*15.0/100.0
         clm(k)%forc_solai(2) = solar*15.0/100.0
         clm(k)%forc_lwrad    = forcing(4)
         clm(k)%forc_u        = forcing(5)
         clm(k)%forc_v        = forcing(6)
         clm(k)%forc_pbot     = forcing(7)
         prcp                 = forcing(8)
         if ( lis%f%force == 5 ) then
            snow              = forcing(9)
            clm(k)%forc_rain   = prcp
            clm(k)%forc_snow   = snow
         endif
...
!          clm(k)%forc_th  = clm(k)%forc_t * (clm(k)%forc_psrf/clm(k)%forc_pbot)**(rair/cpair)
          ...
          forc_vp  = clm(k)%forc_q*clm(k)%forc_pbot / (0.622+0.378*clm(k)%forc_q)
          clm(k)%forc_rho = (clm(k)%forc_pbot-0.378*forc_vp) / (rair*clm(k)%forc_t)
...
! --------------------------------------------------------------------
#endif
    print*, 'not supposed to be in interpa2si..'
#if 0 

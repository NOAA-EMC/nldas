...
!=== Read Active Archive File ============================================
      print*,'DBG: hyssibrst -- in hyssibrst',' (',iam,')'

      if (masterproc) then 
         if (LIS%O%STARTCODE.EQ.3) then
...
!   Check for Grid Space Conflict
            IF (lnc.NE.LIS%D%lnc.OR.lnr.NE.LIS%D%lnr) THEN
               WRITE(*,*) hyssibdrv%HYSSIB_RFILE, &
                          'Grid space mismatch - HY-SSiB HALTED'
               STOP
            ENDIF
...
!--  testing for tile definition conflict.
            SIB_VEGT = TILE%VEGT
            do t=1,nch
               call HYSSIB_MAPVEGC(SIB_VEGT(t))
            enddo
...
!-- Check for tile definition conflict
            ...
            DO L=1,3
               READ(40) TMPTILE
               HYSSIB%WWW(L) = TMPTILE
            ENDDO
            DO L=1,2
               READ(40) TMPTILE
               HYSSIB%CAPAC(L) = TMPTILE
            ENDDO
            DO L=1,2
               READ(40) TMPTILE
               HYSSIB%SNOW(L) = TMPTILE
            ENDDO
            READ(40) HYSSIB%SGFG
            READ(40) HYSSIB%SDENS

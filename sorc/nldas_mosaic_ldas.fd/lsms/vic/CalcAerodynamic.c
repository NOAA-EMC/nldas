...
/*****************************************************************************
  ...
  Required     :
    ...
    char OverStory - flag for presence of overstory.  Only used if NVegLayers 
                     is equal to 1
    float Zref[0]     - Reference height for windspeed
    ...
    float *Ra      - Vector of length 3, with aerodynamic resistance values.  
                     ...
                     float *Ra) 
{
  ...
  else
  {
    ...
    Zt = Trunk * Height;
    //    printf("B4..%f %f %f %f \n",Zt,Zw,Z0_Lower,Z0_Upper);
    if ( Zt < (Z0_Lower+d_Lower) ) 
    {
      fprintf(stderr,"ERROR: Trunk space height below \"center\" of lower boundary\n");

...
static char vcid[] = "$Id: soil_conduction.c,v 4.1 2000/05/16 21:07:16 vicadmin Exp $";
...
/** Start of changes -- JS **/
/* dz_node[nidex+1] is not defined for (nidx+1) = Nnodes and an error 
   may occur at run-time. therefore ensure that dz_node[Nnodes] is set to zero explicitly somewhere */
...
/** Start of changes -- JS **/
/* check that the value is positive before raising it to a power */
/* this occurs when T < KELVIN, which shouldn't really be possible anyway */
/* and may be a problem elsewhere in the code */ 
  unfrozen = 0.0;
  tmp = (-Lf * T) / (T + KELVIN) / (9.18 * bubble / 100.);
  if(tmp > 0.0) {
    unfrozen = max_moist * pow(tmp, -(2.0 / (expt - 3.0)));
/*  unfrozen = max_moist * pow((-Lf * T) / (T
      + KELVIN) / (9.18 * bubble / 100.), -(2.0 / (expt - 3.0)));*/
  }
/** End of changes -- JS **/

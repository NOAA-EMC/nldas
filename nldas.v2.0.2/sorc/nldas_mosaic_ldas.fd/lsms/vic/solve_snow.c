...
*********************************************************************/
  ...
  rainonly      = calc_rainonly(air_temp, prec, mu);
  //printf("RAINF ..%f %f %f \n",prec, rainonly,air_temp);
  //	 gauge_correction[RAIN]);
  snowfall[WET] = gauge_correction[SNOW] * (prec - rainonly);
  rainfall[WET] = gauge_correction[RAIN] * rainonly;
  snowfall[DRY] = 0.;
  rainfall[DRY] = 0.;
  //  printf("RAINF ..%f %f %f \n",air_temp,snowfall[WET],rainfall[WET]);
  ...
  if((snow->swq > 0 || snowfall[WET] > 0.
	...
	* (snow->surf_temp+KELVIN) * (snow->surf_temp+KELVIN);
	...
	if( snowfall[WET] > 0. || snow->snow_canopy > 0. ) {
	  ...
	  snow_intercept((float)dt, 1., veg_lib[veg_class].LAI[month-1], 
			 ...
			 &tmp_energy_val, month, hour);
          /* test for very small snowfall as this may cause instabilities 
	...
	(*rad)          = (*net_short) + longwave - STEFAN_B 
	  ...
	  * (snow->surf_temp+KELVIN) * (snow->surf_temp+KELVIN);
      ...
      if((full_energy_flag==1) || (frozen_soil_flag==1)) grnd_temp = energy->T[0];
      else 
	grnd_temp = air_temp;
...
#if 1
/** JS -- start of changes **/
/* estimate the snow depth when it first snows so that the snow flux can be computed */
if(snow->swq == 0.0) {
  snow->density = snow_density(day_in_year, snowfall[WET], air_temp,
                                     snow->swq, snow->depth, snow->coldcontent,
                                     (double)dt, snow->surf_temp);
  snow->depth = 1000. * (snowfall[WET] / 1000.) / snow->density;
}
/** JS -- end of changes **/
#endif
      snow_melt(soil_con, iveg, wind_h+soil_con->snow_rough, 
		aero_resist[2], Le[0], snow, (float)dt, 0.00, 

...
!clm-
!       clm(k)%forc_swc1    = inf       !Layer 1 (0-10 cm) soil water content (% capacity for GEOS 
!       clm(k)%forc_sdepth  = inf       !Model liquid equivalent snow depth (kg/m2) (same for GEOS)
...
! Temperatures
       ...
       clm(k)%t_soisno(-nlevsno+1:0) = 0      !snow temperature (Kelvin)
       clm(k)%t_soisno(1:nlevsoi)    = 0      !soil temperature (Kelvin)
...
!       clm(k)%frm   = inf         !total maintenance respiration (umol CO2 /m**2/s)
!       clm(k)%frmf  = inf         !leaf maintenance respiration  (umol CO2 /m**2 /s)
!       clm(k)%frms  = inf         !stem maintenance respiration  (umol CO2 /m**2 /s)
!       clm(k)%frmr  = inf         !root maintenance respiration  (umol CO2 /m**2 /s)
!       clm(k)%frg   = inf         !growth respiration (umol CO2 /m**2 /s)
!       clm(k)%fmicr = inf         !microbial respiration (umol CO2 /m**2 /s)
!       clm(k)%fco2  = inf         !net CO2 flux (umol CO2 /m**2 /s) [+ = to atm]
!       clm(k)%dmi   = inf         !total dry matter production (ug /m**2 /s)

! dust model

!       clm(k)%flx_mss_vrt_dst(1:ndst) = inf !surface dust emission (kg/m**2/s) [ + = to atm]
!       clm(k)%vwc_thr                 = inf !threshold soil moisture based on clay content
!       clm(k)%mss_frc_cly_vld         = inf ![frc] Mass fraction clay limited to 0.20
!       clm(k)%mbl_bsn_fct             = inf

! voc model

!       clm(k)%vocflx(1:nvoc) = inf          !VOC flux [ug C m-2 h-1]
...
!       clm(k)%snowliq         = inf                !average snow liquid water
       ...
       clm(k)%frac_iceold(-nlevsno+1:nlevsoi)=inf  !fraction of ice relative to the total water (new)
...
!       clm(k)%albi(1:numrad)  = inf !surface albedo (diffuse)                    
       ...
       clm(k)%surfalb         = inf !instantaneous all-wave surface albedo
!       clm(k)%surfalb_old     = inf !previous instantaneous all-wave surface albedo
!       clm(k)%snoalb_old      = inf !previous instantaneous all-wave snow albedo
...
!       clm(k)%bm_inc = inf
!       clm(k)%afmicr = inf
!       clm(k)%firelength = inf
       clm(k)%wf = inf
!       clm(k)%litterag = inf
!       clm(k)%litterbg = inf
!       clm(k)%cpool_fast = inf
!       clm(k)%cpool_slow = inf
!       clm(k)%k_fast_ave = inf
!       clm(k)%k_slow_ave = inf
!       clm(k)%litter_decom_ave = inf
...
!       clm(k)%qmelt  = inf ! snow melt [mm/s]
       ...
       clm(k)%totfsa=0.              ! solar absorbed solar radiation [W/m2]
       clm(k)%toteflx_lwrad_net=0.   ! net longwave radiation [W/m2]
       clm(k)%toteflx_lh_tot=0.      ! total latent heat flux [W/m2]
       clm(k)%toteflx_sh_tot=0.      ! total sensible heat flux [W/m2]
       clm(k)%toteflx_soil_grnd=0.   ! ground heat flux [W/m2]
       clm(k)%totqflx_snomelt=0.     ! snowmelt heat flux [W/m2]
!       clm(k)%totsolisbd=0.          ! total downward surface shortwave radiation [W/m2]
!       clm(k)%totforc_lwrad=0.       ! atmospheric infrared (longwave) radiation [W/m2]
       clm(k)%totrain=0.             ! accumulation of rain [mm]
       clm(k)%totsnow=0.             ! accumulation of snow [mm]
       clm(k)%totqflx_evap=0.        ! total evaporation [mm]
       clm(k)%totqflx_surf=0.        ! surface runoff [mm]
       clm(k)%totqflx_drain=0.       ! subsurface runoff [mm]
       clm(k)%totqflx_ecanop=0.      ! interception evaporation [W/m2]
       clm(k)%totqflx_tran_veg=0.
       clm(k)%totqflx_evap_grnd=0.
       clm(k)%totqflx_sub_snow=0.
...
end subroutine clm_varder_init

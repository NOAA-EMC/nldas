...
subroutine clm1_stomata(mpe,       apar,     ei,        ea,      &
                       tgcm,      o2,       co2,                &
                       btran,     rb,       rs,        psn,     &
                       qe25,      kc25,     ko25,      vcmx25,  &
                       akc,       ako,      avcmx,     bp,      &
                       mp,        foln,     folnmx,    c3psn,  clm1) 
...
!=========================================================================
! $Id: clm1_stomata.F90,v 1.1.1.1 2003/02/06 16:10:43 jgottsch Exp $
!=========================================================================
...
! Set constants
  ...
  rsmax0 = 2.e4
  cf = clm1%forc_pbot/(8.314*tgcm)*1.e06 
...
! ci iteration
     ...
     do iter = 1, niter
        ...
        we = 0.5*vcmx*c3psn + 4000.*vcmx*ci/clm1%forc_pbot*(1.-c3psn) 
        psn = min(wj,wc,we) 
        cs = max( co2-1.37*rb*clm1%forc_pbot*psn, mpe )
        a = mp*psn*clm1%forc_pbot*cea / (cs*ei) + bp
        b = ( mp*psn*clm1%forc_pbot/cs + bp ) * rb - 1.
        c = -rb
        if (b >= 0.) then
        ...
        r2 = c/q
        rs = max(r1,r2)
        ci = max( cs-psn*clm1%forc_pbot*1.65*rs, 0. )
     enddo

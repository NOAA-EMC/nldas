...
subroutine clm1_soilwater (vol_liq,    eff_porosity,     qinfl,   sdamp,  &
                          dwat,       hk,               dhkdw,   clm1 )
...
!=========================================================================
! $Id: clm1_soilwater.F90,v 1.1.1.1 2003/02/06 16:10:46 jgottsch Exp $
!=========================================================================
...
! two neighbor layers or liquid content (theta) less than 0.001
  ...
  do j = 1, nlevsoi
     if (      (eff_porosity(j) < clm1%wimp) &
          .OR. (eff_porosity(min(nlevsoi,j+1)) < clm1%wimp) &
          .OR. (vol_liq(j) <= 1.e-3))then
        hk(j) = 0.
        dhkdw(j) = 0.
     else
        s1 = 0.5*(vol_liq(j)+vol_liq(min(nlevsoi,j+1))) / &
            (0.5*(clm1%watsat(j)+clm1%watsat(min(nlevsoi,j+1))))
        s2 = clm1%hksat(j)*s1**(2.*clm1%bsw(j)+2.)
        hk(j) = s1*s2  
        dhkdw(j) = (2.*clm1%bsw(j)+3.)*s2*0.5/clm1%watsat(j)
...
! d(smp)/d(vol_liq), and d(hk)/d(vol_liq).
  ...
  do j = 1, nlevsoi
     ...
     if (clm1%t_soisno(j)>273.16) then

        s_node = max(vol_liq(j)/clm1%watsat(j),0.01)
        s_node = min(1.,s_node)
        smp(j) = -clm1%sucsat(j)*s_node**(-clm1%bsw(j))
        smp(j) = max(clm1%smpmin, smp(j))        ! Limit soil suction
        dsmpdw(j) = -clm1%bsw(j)*smp(j)/(s_node*clm1%watsat(j))
...
! Unit 1 Joule = 1 (kg m2/s2), J/kg /(m/s2) ==> m ==> 1e3 mm 
        ...
        smp(j) = 1.e3 * 0.3336e6/9.80616*(clm1%t_soisno(j)-273.16)/clm1%t_soisno(j)
        smp(j) = max(clm1%smpmin, smp(j))        ! Limit soil suction
        dsmpdw(j) = 0.
...
! Node j=1
  ...
  rmx(j) =  qin - qout - clm1%qflx_tran_veg*clm1%rootr(j)
  amx(j) =  0.
  bmx(j) =  dz(j)*(sdamp+1./clm1%dtime) + dqodw1
  cmx(j) =  dqodw2
...
! Nodes j=2 to j=nlevsoi-1
  ...
  do j = 2, nlevsoi - 1
     ...
     rmx(j) =  qin - qout - clm1%qflx_tran_veg*clm1%rootr(j)
     amx(j) = -dqidw0
     bmx(j) =  dz(j)/clm1%dtime - dqidw1 + dqodw1
     cmx(j) =  dqodw2
  enddo
...
! Solve for dwat
  ...
  call clm1_tridia (nlevsoi ,amx ,bmx ,cmx ,rmx ,dwat)

end subroutine clm1_soilwater

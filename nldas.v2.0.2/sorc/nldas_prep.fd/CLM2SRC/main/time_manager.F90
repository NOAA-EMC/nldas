...
module time_manager
   ...
   use mfm_timemgmt, only: &
      mfm_errhandlersettype, mfm_err_return, mfm_errprint, mfm_success, &
      mfm_time, mfm_timeinit, mfm_timeget, mfm_timegetdays, &
        mfm_timeincrement, mfm_timedecrement, &
      mfm_date, mfm_dateinit, mfm_gregorian, mfm_no_leap, mfm_dateget, &
        mfm_dateincrementsec, mfm_dateincrementday, mfm_datedecrement, &
        mfm_datediff, mfm_dategetfltdayofyear, &
      mfm_timemgr, mfm_timemgrinit, mfm_timemgradvance, mfm_timemgrgetnstep, &
        mfm_timemgrgetstepsize, mfm_timemgrgetstartdate, mfm_timemgrgetbasedate, &
        mfm_timemgrlaststep, mfm_timemgrgetcurrdate, mfm_timemgrgetprevdate, mfm_dateislater, &
        mfm_timemgrrestartwrite, mfm_timemgrrestartread
...
! Private module data
   ...
   type(mfm_timemgr) :: tm_id          ! time manager ID
...
! Local variables
   ...
   type(mfm_time) :: step_size   ! timestep size
   type(mfm_date) :: start_date  ! start date for run
   type(mfm_date) :: stop_date   ! stop date for run
   type(mfm_date) :: ref_date    ! reference date for time coordinate
...
! Initialize timestep size.
   ...
   step_size = mfm_timeinit(0, dtime, rc)
   call chkrc(rc, sub//': error return from mfm_timeinit: setting step_size')
...
! Initialize start date.
   ...
   start_date = mfm_dateinit(cal_type, start_ymd, start_tod, rc)
   call chkrc(rc, sub//': error return from mfm_dateinit: setting start_date')

! Initialize reference date for time coordinate.

   if ( ref_ymd /= uninit_int ) then
      ref_date = mfm_dateinit(cal_type, ref_ymd, ref_tod, rc)
   else
      ref_date = mfm_dateinit(start_date, rc)
   end if
   call chkrc(rc, sub//': error return from mfm_dateinit: setting ref_date')

! Initialize stop date.

   if ( stop_ymd /= uninit_int ) then
      stop_date = mfm_dateinit(cal_type, stop_ymd, stop_tod, rc)
   else if ( nestep /= uninit_int ) then
      if ( nestep >= 0 ) then
         stop_date = mfm_dateincrementsec(start_date, dtime*nestep, rc)
      else
         stop_date = mfm_dateincrementday(start_date, -nestep, rc)
      end if
   else if ( nelapse /= uninit_int ) then
      if ( nelapse >= 0 ) then
         stop_date = mfm_dateincrementsec(start_date, dtime*nelapse, rc)
      else
         stop_date = mfm_dateincrementday(start_date, -nelapse, rc)
      end if
   else
...
! Initialize a time manager.
   ...
   tm_id = mfm_timemgrinit(step_size, start_date, stop_date, ref_date, rc)
   call chkrc(rc, sub//': error return from mfm_timemgrinit')

! Calculation of ending timestep number (nestep) assumes a constant stepsize.

   ntspday = 86400/dtime
   diff = mfm_timeinit()
   call mfm_datediff(start_date, stop_date, diff, islater, rc)
   call chkrc(rc, sub//': error return from mfm_datediff calculating nestep')
   call mfm_timeget(diff, ndays, nsecs, rc)
   call chkrc(rc, sub//': error return from mfm_timeget calculating nestep')
...
! Local variables
   ...
   type(mfm_date) :: start_date  ! start date for run
   type(mfm_date) :: stop_date   ! stop date for run
   type(mfm_date) :: curr_date   ! date of data in restart file
   logical :: islater
   integer :: ymd, tod
...
! Initialize calendar type.
   ...
   if ( rst_type == mfm_no_leap ) then
      calendar = 'NO_LEAP'
   else if ( rst_type == mfm_gregorian ) then
      calendar = 'GREGORIAN'
   else
...
! Initialize start date.
   ...
   start_date = mfm_dateinit(rst_type, rst_start_ymd, rst_start_tod, rc)
   call chkrc(rc, sub//': error return from mfm_dateinit: setting start_date')

! Initialize current date.

   curr_date = mfm_dateinit(rst_type, rst_curr_ymd, rst_curr_tod, rc)
   call chkrc(rc, sub//': error return from mfm_dateinit: setting curr_date')

! Initialize stop date.

   if ( stop_ymd /= uninit_int ) then
      stop_date = mfm_dateinit(rst_type, stop_ymd, stop_tod, rc)
   else if ( nestep /= uninit_int ) then
      if ( nestep >= 0 ) then
         stop_date = mfm_dateincrementsec(start_date, dtime*nestep, rc)
      else
      ...
      if ( nelapse >= 0 ) then
         stop_date = mfm_dateincrementsec(curr_date, dtime*nelapse, rc)
      else
         stop_date = mfm_dateincrementday(curr_date, -nelapse, rc)
      end if
   else
      stop_date = mfm_dateinit(rst_type, rst_stop_ymd, rst_stop_tod, rc)
   end if
   call chkrc(rc, sub//': error return setting stop_date')

! Check that stop date is later than current date.

   call mfm_dateislater(curr_date, stop_date, islater, rc)
   call chkrc(rc, sub//': error return from mfm_dateislater: comparing start and stop dates')
   if ( .not. islater ) then
      write(6,*)sub,': stop date must be specified later than current date: '
      call mfm_dateget(curr_date, ymd, tod)
      write(6,*)' Current date (ymd tod): ', ymd, tod
      call mfm_dateget(stop_date, ymd, tod)
      write(6,*)' Stop date (ymd tod): ', ymd, tod
      call endrun
   end if
   call mfm_dateget(stop_date, rst_stop_ymd, rst_stop_tod)

! Restart a time manager.

   tm_id = mfm_timemgrrestartread(rst_type, rst_nstep, rst_step_days, rst_step_sec, rst_start_ymd, &
                                  rst_start_tod, rst_stop_ymd, rst_stop_tod, rst_ref_ymd, rst_ref_tod, &
                                  rst_curr_ymd, rst_curr_tod, rc)
   call chkrc(rc, sub//': error return from mfm_timemgrrestartread')
...
!-----------------------------------------------------------------------------------------
   ...
   call mfm_timemgrrestartwrite(tm_id, type, nstep, step_days, step_sec, &
                                start_ymd, start_tod, stop_ymd, stop_tod, ref_ymd, &
                                ref_tod, curr_ymd, curr_tod, rc)
   call chkrc(rc, sub//': error return from mfm_timemgrrestartwrite')

   write(6,*)' ********** Time Manager Configuration **********'

   if ( type == mfm_no_leap ) then
      cal = 'NO_LEAP'
   else if ( type == mfm_gregorian ) then
      cal = 'GREGORIAN'
   end if
...
!-----------------------------------------------------------------------------------------
   ...
   call mfm_timemgradvance(tm_id, rc)
   call chkrc(rc, sub//': error return from mfm_timemgradvance')
...
!-----------------------------------------------------------------------------------------
   ...
   call mfm_timemgrgetstepsize(tm_id, days, seconds, rc)
   call chkrc(rc, sub//': error return from mfm_timemgrgetstepsize')
...
!-----------------------------------------------------------------------------------------
   ...
   get_nstep = mfm_timemgrgetnstep(tm_id, rc)
   call chkrc(rc, sub//': error return from mfm_timemgrgetnstep')
...
!-----------------------------------------------------------------------------------------
   ...
   date = mfm_timemgrgetcurrdate(tm_id, rc)
   call chkrc(rc, sub//': error return from mfm_timemgrgetcurrdate')

   if (present(offset)) then
      if (offset > 0) then
         date = mfm_dateincrementsec(date, offset, rc)
         call chkrc(rc, sub//': error incrementing current date')
      else if (offset < 0) then
         off = mfm_timeinit(0, -offset, rc)
         call chkrc(rc, sub//': error setting offset time type')
         date = mfm_datedecrement(date, off, rc)
         call chkrc(rc, sub//': error decrementing current date')
      end if
   end if

   call mfm_dateget(date, ymd, tod, rc)
   call chkrc(rc, sub//': error return from mfm_dateget')
   yr = ymd/10000
...
!-----------------------------------------------------------------------------------------
   ...
   date = mfm_timemgrgetprevdate(tm_id, rc)
   call chkrc(rc, sub//': error return from mfm_timemgrgetprevdate')
...
!-----------------------------------------------------------------------------------------
   ...
   date = mfm_timemgrgetstartdate(tm_id, rc)
   call chkrc(rc, sub//': error return from mfm_timemgrgetstartdate')
...
!-----------------------------------------------------------------------------------------
   ...
   date = mfm_timemgrgetbasedate(tm_id, rc)
   call chkrc(rc, sub//': error return from mfm_timemgrgetbasedate')
...
! Local variables
   ...
   integer :: rc
   type(mfm_date) :: cdate, rdate
   type(mfm_time) :: diff
   logical :: islater
!-----------------------------------------------------------------------------------------

   cdate = mfm_timemgrgetcurrdate(tm_id, rc)
   call chkrc(rc, sub//': error return from mfm_timemgrgetcurrdate')

   rdate = mfm_timemgrgetbasedate(tm_id, rc)
   call chkrc(rc, sub//': error return from mfm_timemgrgetbasedate')

   call mfm_datediff(rdate, cdate, diff, islater, rc)
   call chkrc(rc, sub//': error return from mfm_datediff')

   call mfm_timeget(diff, days, seconds, rc)
   call chkrc(rc, sub//': error return from mfm_timeget')
...
!-----------------------------------------------------------------------------------------
   ...
   get_curr_calday = mfm_dategetfltdayofyear(date, rc)
   call chkrc(rc, sub//': error return from mfm_dategetfltdayofyear')
...
!-----------------------------------------------------------------------------------------
   ...
   nstep = mfm_timemgrgetnstep(tm_id, rc)
   call chkrc(rc, sub//': error return from mfm_timemgrgetnstep')
...
!-----------------------------------------------------------------------------------------
   ...
   is_last_step = mfm_timemgrlaststep(tm_id, rc)
   call chkrc(rc, sub//': error return from mfm_timemgrlaststep')
...
!-----------------------------------------------------------------------------------------
   ...
   call mfm_timemgrrestartwrite(tm_id, rst_type, rst_nstep, rst_step_days, rst_step_sec, &
...
subroutine chkrc(rc, mes)
   ...
   if ( rc == mfm_success ) return
   write(6,*) mes
   call mfm_errprint(rc)
   call endrun

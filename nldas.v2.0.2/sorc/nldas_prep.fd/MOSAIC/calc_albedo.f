C  This is a subroutine for the LDAS driver
c  i've commented out the program part of the this file.
c  because the driver only needs the subroutine.
c  -Jared
c
c
C      	This program computes 4 albedos given :
C         LAI, Greenness fraction, Cosine of the Zenith Angle,
C         Snow depth (set to 0 now since snow-related albedo 
C         calculations have been disabled in this subroutine and 
C         are taken care of in the main code of Mosaic), Veg type,
C         IRUN (set to 1) and Canopy Temp. (set to 0 b/c this 
C         is only needed for snow related calculations that have
C         been disabled in this subroutine
C
C     	Current version of mosaic uses avisdr,anirdr,avisdf,anirdf 
C         that are all set to the albedo taken from EDAS data
C       Using this subroutine, these four parameters can be
C         individually computed and passed back to the main model.
C
C	This subroutine is a modified version of the one written
C	  by Randy Koster / GSFC---it was modified by 
C         Brian Cosgrove / GSFC


C	REAL AVISDR,ANIRDR,AVISDF,ANIRDF,VLAI
C        REAL VGRN,ZTH,SNW,tc
C
C        INTEGER ITYP,irun
C
C
...
C        VGRN=0.6
C
C        call SIBALB(AVISDR, ANIRDR, AVISDF, ANIRDF,
C     &              VLAI, VGRN, ZTH, SNW, ITYP, IRUN,tc)
C
C        print *,'AVISDR, ANIRDR, AVISDF, ANIRDF'
C        print *,'VLAI, VGRN, ZTH, SNW, ITYP, IRUN'
C
C        print *,AVISDR, ANIRDR, AVISDF, ANIRDF
C        print *,VLAI, VGRN, ZTH, SNW, ITYP, IRUN
C        end
...
C
	SUBROUTINE oldSIBALB (
     ...
     I			 VLAI, VGRN, ZTH, SNW, ITYP, IRUN, TC
     &       		)
...
C MOSAIC ITYP: Vegetation type as follows:
...
C UMD ITYP: Vegetation type as follows:
...
C    GROUNDCOVER (ITYP=4); GREEN=0.33; LAI=.5-7
	DATA (ALVDRold (I, 1, 4), I = 1, 14)
     `	  /0.2436, 0.2470, 0.2486, 0.2494, 0.2498, 0.2500, 2*0.2501,
     `		6*0.2502
     `	  /
C    GROUNDCOVER (ITYP=4); GREEN=0.67; LAI=.5-7
...
C----------------------------------------------------
C	This section commented out b/c snow albedo already accounted
c	for in main Mosaic code.
c
c	If this part of the code will be used, then the SNWALBold
c	and SNWMIDold need to be mapped to UMD values, and the 
c	section of code below needs to be un-commented out.

C      EXP039: ALLOW REDUCTION IN ALBEDO WHEN SNOW IS CLOSE TO MELTING.
C      (FROM SIB). WE USE 1K (INSTEAD OF .1) BECAUSE RADIATION
C      IS CALLED EVERY 3 HOURS.

c	  IF (SNW (I) .GT. ZERO) THEN
c	   FAC = SNW(I) / (SNW(I) + SNWMID(ITYP(I)))

c           WRMFAC=1.0
c           IF(TC(I) .GT. TICE-1.0) WRMFAC=0.6

c	   AVISDR(I) = AVISDR(I) +
c     &  (SNWALB(1,ITYP(I))*WRMFAC - AVISDR(I)) * FAC
c	   ANIRDR(I) = ANIRDR(I) +
c     &  (SNWALB(2,ITYP(I))*WRMFAC - ANIRDR(I)) * FAC
c	   AVISDF(I) = AVISDF(I) +
c     &  (SNWALB(3,ITYP(I))*WRMFAC - AVISDF(I)) * FAC
c	   ANIRDF(I) = ANIRDF(I) +
c     &  (SNWALB(4,ITYP(I))*WRMFAC - ANIRDF(I)) * FAC
c	  ENDIF
